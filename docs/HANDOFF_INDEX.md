...
| `COACHMARKS_ENABLED` | `src/components/flow/GestureSpotlight.tsx` | `false` right now: every Coachmark (Explore's For You -> Schools tour, the action-icon hints on the reel and Career Detail) is *off* for demos. Flip to `true` to bring them all back exactly as they were; the per-device "seen" flags are untouched. The Flow Lab used to force its own coachmarks on regardless of this flag; that escape hatch (`demoForce`) is gone (25 Sept 2026 -- Joshua: "avoid instructional pop-ups unless absolutely necessary... the interface itself should make the next action obvious"), so this flag is now the only switch anywhere in the app. |
...
- **Counselor Dashboard** (`/counselor`; `/counselor/login` and `/counselor/signup` now just redirect there) -- `src/app/counselor/`, `src/components/counselor/`. A genuinely separate product from the student app (own shell, no shared chrome), reachable only from the hamburger's "Counselor Demo" quick link. No sign-in or sign-up (direct instruction, 25 Sept 2026): the account record in localStorage (name, school, role from Settings) still shapes the shell but never gates it. Reads real data where it exists (one live student, from this browser's own localStorage) inside an otherwise seeded 119-student roster -- see `src/lib/counselorRoster.ts`'s own header comment before treating any one row as real aggregate data, and `docs/AI_HANDOFF.md`, 22 Sept 2026 for the full rationale. Originally a 1:1 port of a Replit reference (structure/copy/vocabulary matched, our own visual execution); it's now being improved screen by screen, and every change to content, data, or structure away from that original port is logged in `docs/COUNSELOR_DASHBOARD_REFERENCE_DEVIATIONS.md` -- read that file before assuming this dashboard still matches the reference anywhere.
...
- **Flow Lab** (`/flow-lab`, one quick link in the hamburger under its own "Match flow lab" divider, `LAB_LINKS` in `chrome.tsx`) -- `src/app/flow-lab/`, `src/components/flow-lab/`. An isolated, replayable place to play Joshua's Build -> Mini Explore -> Saved -> Rank -> My Profile proposal: it reads the catalog and the real Build's answers (worlds, subjects, college/trades) read-only and writes only `dreamari:flowlab:*` keys, so nothing about the live demo's Build, `/match-grid` or `/profile` changes (direct instruction, 24 Sept 2026: "NOTHING SHOULD CHANGE IN THE DEMO"). Reworked 25 Sept 2026 after Joshua reviewed the first build and asked to drop the second flow (v3) and simplify v2: no coachmarks anywhere (the persistent hint line under each header teaches the screen instead); Mini Explore opens on the student's first-chosen (strongest) world with the second as another tab, no "For You" tab; careers load in on scroll (`RevealGrid`/`useRevealCount` in `shared.tsx`) instead of a "Six more" button; "Explore more" renamed "Explore all". Remove the route and the folder once a flow is chosen and built for real.
