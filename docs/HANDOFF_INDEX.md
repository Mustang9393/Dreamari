...
| `COACHMARKS_ENABLED` | `src/components/flow/GestureSpotlight.tsx` | `false` right now: every Coachmark (Explore's For You -> Schools tour, the action-icon hints on the reel and Career Detail) is *off* for demos. Flip to `true` to bring them all back exactly as they were; the per-device "seen" flags are untouched. The Flow Lab (`/flow-lab`) is the one exception: it passes `demoForce` on its own coachmarks so they run while this is `false`; nothing in the demo app passes it. |
