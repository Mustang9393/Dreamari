...
- **Counselor Dashboard version chip** (`v1`/`v2`, `?v=2`, remembered in localStorage `dreamari:counselor-version`) -- `src/components/counselor/version.tsx`, switched per view in `CounselorApp.tsx`, docked bottom-center by `shell.tsx`. v1 is the reference port plus the passes already landed on it; v2 is a fork (`src/components/counselor/v2/`, one file per screen) where the 24 Sept 2026 audit's changes land, so the two can be compared live. Demo-only plumbing; whichever build wins should be collapsed back into one set of files.
