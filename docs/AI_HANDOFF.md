...
- 2026-09-03 (night): **Event names + stacked lockup.** All six events renamed to the Replit's "Dream Opportunity × Company City" shape (JA event: "Junior Achievement × Goldman Sachs NYC"). `EventMarks` is now a two-line lockup (`LOCKUP` md 168×48 L14, lg 232×66 L19): lead mark + small × on line one, partner on line two, left-anchored; a JA host puts JA on top and Goldman Sachs under it while the card colour still follows the host. Verified at 375 and desktop on the Events list and the event banner. Not written down anywhere as a CEO rule; inferred from the Replit's own board names.
