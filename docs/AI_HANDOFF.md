...
- 2026-09-04: **College page data viz (pushed to main/demo).** `src/components/colleges/viz.tsx`: `HBars` (one-hue bars, value at tip, optional `marker` reference line; `ref` is a reserved prop name, do not use it), `SplitBar` (two shades, 2px gap, legend with shares), `RangeBar` (SAT 400-1600, ACT 1-36), `DotGrid` (100 dots), `MeterRow`. Detail sections rebuilt around them; duplicates removed (undergrads, admit rate and finish live only in At a glance). Dataviz skill rules followed: sequential single hue, text in text tokens, no dual axis, legend for two-series splits.
