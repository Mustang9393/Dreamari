...
## HUD wears the career's own world color (Chandu, 7 Sept 2026)
The reputation gauge (ring, number, star) and the progress bar used a generic tier color (`BAND_COLOR`, keyed by Cautious/Respected/etc.) that had nothing to do with the career -- two of its four tiers happened to borrow Business & Money's gold and Building & Construction's orange as stand-ins, which made a Health & Medicine sim's HUD read orange/gold instead of its own teal. Both now render in `WORLD_COLORS[simulation.world]` (`accent`) instead: Nursing is teal (`--world-health-medicine`), IB is gold (`--world-business-money-office`), same as every other HUD element (level label, checkpoint dots, Express outcome highlight) already did. `BAND_COLOR` still drives the end-of-level report card (Trusted/Respected/Cautious/At Risk) -- that screen is a pass/fail verdict, not in-play chrome, so it keeps the status-color signal. Verified live on both `/play/registered-nurse` and `/play/investment-banking`, Full and Express.
