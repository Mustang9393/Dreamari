...
The header (cover, portrait, name, tier, role, company mark, Follow, three numbers, story, verification) is the anchor. Under it, one panel holds four ruled sections in a fixed order, each with an eyebrow naming the dimension and the section title: How you can engage (Ask Me), What I share (My Posts), Where I participate (Communities, as rows with the community's accent mark, name, students and companies), My background (About Me: education, how I got here, can help with). No section is its own floating card. `ProfileSection` in `ProProfile.tsx`.
