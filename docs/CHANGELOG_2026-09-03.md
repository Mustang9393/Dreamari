...
- App pages: one left margin everywhere. Home, Explore, Play, Connect, Profile and Colleges share the same 1440px column with the same side padding, so titles line up at 56px from the edge on desktop and 20px on phones. Play and Profile were narrower and centred, which pushed their content 120px further right on wide screens. Connect's reading views (a board, a thread, a profile) keep their 880px column but sit on the same left rail instead of centring.
