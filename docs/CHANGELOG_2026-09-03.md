...
- Community cards: the stat boxes and company marks sit on a light blur instead of a dark fill, so they read as part of the photo rather than chips; Open is a solid dark rounded button with an outward arrow, the one solid object on the card, no blue.
