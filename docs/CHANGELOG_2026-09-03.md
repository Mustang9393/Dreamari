...
- Connect: the nine new professionals' portraits now load in the volunteer picker row too (the row has its own photo map, which had not been updated).
- Connect, professional profiles (student view): the identity card wears the company's own colours, the same treatment as the company card on the volunteer dashboard, with the company mark in the brand's ink. The professional's activity badge (Diamond, Gold or Silver volunteer) sits beside their name; a professional on a break shows none.
- Connect, professional profiles: the brand colour is a wash over the app's dark rather than full strength, with white type on every company; the brand shows at full strength only as a soft corner glow. The Follow button takes the company's colours instead of the app blue; dark brands (Nike, JPMorgan Chase, Morgan Stanley) get an inverted white button so it never sinks into the wash.
- App pages: one page title everywhere. Home, Explore, Play, Connect, Profile and Find a college all use the same all caps display title at the same size, from the same shared class; Home and Profile now show theirs instead of hiding it. "Find your community" is now "Connect".
- App pages: one side padding everywhere (20px on phones, 56px from tablet up). Home, Explore, Play, Connect and Colleges share the 1440px column so their titles line up. Reading pages stay centred at their own width because a reading column pinned to the left rail sat awkwardly on wide screens: Profile at 1200px, Connect boards, threads and professional profiles at 880px, career and college details as before.
- Career detail: opens at the top of the page. Arriving from a rail deep in Explore or Home kept the previous scroll position, so the page opened mid-way.
- Profile in light mode: the groups inside the tab card (Overview doorways, Do this next, the Plan and Report panels' rows) used a fixed dark surface with white hairlines, which turned them into unreadable dark blocks on the light page. They now use theme-aware inset tokens: a darker step on dark, a lighter step on light. The tab card's border and the plan rows' rules follow the theme too. Every tab checked in light: Overview, Top Three, Plan, Report, Resume, Archive, Event Stubs.
- Light mode, every page: a WCAG contrast pass over Home, Explore (both tabs), Play, Connect (landing, event board, professional profile, volunteer dashboard), Career detail, Profile, Find a college, College detail, Career reports and the landing page. Every text on a theme surface now clears AA (4.5:1 body, 3:1 large). Fixes: career detail's pay-by-state state names and list rows were white on the light page (now the theme's text colour); the home carousel's eyebrow and chip were the theme's dark tint on the dark photo (now lifted toward white); the Glossary Game badge's ink and the ghost buttons over photos were a shade too faint; the college detail title over its photo now stays white in both themes.
- Light mode, world colours: the amber and every other world colour went muddy because they were mixed with black. Each is now the same hue at full saturation, darkened only until it clears 4.5:1 on the light card, so amber reads as a deep gold, not brown. Chart 3 follows.
- Light mode, hairlines: dividers and panel borders that were hard-coded white (career detail panels and popovers, the search sheet, college compare table and viz rows, profile dialog, volunteer dashboard stats, Connect thread replies, professional profile panels) now use the theme's border token, so they show in light mode instead of vanishing.
- Landing page on tablets: Build and Match (and every chapter) took the desktop rule of centring the whole row, which parked the title mid-screen with a void above it. Tablets now use the phone layout: the graphic is centred and the title and caption hang directly above it.
- Landing, Play into Connect (mobile): tapping "Enter Community" right after the auto-scroll from Play used to snap the page back to Play. The two Connect screens (overview, post thread) are now both always mounted and crossfade instead of swapping, so the section's height never changes at the moment of the tap and the scroll-snap browser behaviour has nothing to react to.
- Connect, community cards (mobile): two company marks now always show before the "+N" chip (was one). Tapping "+N" opens a small panel with the rest and a close button; it also closes on a tap outside or on scroll, instead of only on hover.
- Connect, community board: the ask composer now sits right under the board's header, above the questions feed, instead of after every existing thread.
- App quick-links menu and the landing page's mobile menu: both now close as soon as the page scrolls, not only on an explicit tap to close.
- Connect, event boards: redesigned from one long stacked scroll into six tabs (Questions, Posts, Insights, Resources, People, About), same pattern the community boards already use. Questions is the tab you land on, with the ask composer at its very top. Posts holds the event account's official recap; Resources holds the panel materials and event photos; People lists the real verified professionals who work at the event's host/partner (matched from Connect's own professional directory, not invented) plus the attendee count with no invented names; About holds the host, topics and dates.
- Explore > Browse, Videos Inside Leading Companies: reordered per Joshua Pierce (Slack, 5 Sept 2026) to Mars, JPMorgan Chase, EY, AT&T, WildBrain, Kellogg's, and EY's second clip is no longer back to back with its first in this rail (EY now appears once here; both video files are untouched, just the duplicate listing is gone).
- Explore > Browse, company video rail: the JPMorgan Chase mark was actually "J.P.Morgan" alone (that division's own logo, missing "Chase" entirely) — replaced with the real current wordmark "JPMorganChase" (Wikimedia Commons, 2024). No public JPMorganChase brand-guidelines page exists to check monochrome usage rules against; the new asset is a single-color mark, same as every other logo in this rail, so it already recolors correctly in both themes.
- Explore > Browse, Mars/Kellanova card: swapped the poster still for a clean title-card frame from the same clip so the Kellanova product branding is no longer visible on the thumbnail; the Mars wordmark chip still identifies it.
- Explore > Browse, company video rail in light mode: every logo was hardcoded white, which disappeared against light mode's near-white poster scrim. Logos now use the same ink token the poster title already did, so they stay legible in both themes.
- Explore > Browse, company video rail: swapped in the designed IG-reel covers for JPMorgan Chase, EY, AT&T and Kellogg's (Mars and WildBrain keep their existing covers). Each clip's title now matches the words on its own cover (JPMorgan Chase: "London Office Tour", EY: "Inside Cybersecurity Consulting", AT&T: "Headquarters Office Tour").
- Explore > Browse, company video rail: the play button moved from dead center (where it sat right over people's faces on nearly every cover) to a small badge in the top-right corner, so the actual subject of each thumbnail stays visible.
- Explore > Browse, company video rail: dropped the separate title-text line on every card, since every cover now bakes its own title in (the scrim now carries only the company mark, and is shorter to match). Built a matching "WildBrain / Office Tour" cover from a clean frame of the real office wall logo, since no cover was supplied for it (same style as the other four designed covers).
- Explore > Browse, company video lightbox: the close X now sits right on the video card's own top-right corner instead of the screen's corner, so it's obvious and close at hand on a wide desktop viewport where the card sits far from the actual screen edge. Backdrop click and Escape still close it too.
- Landing page order: swapped Play and Explore per Joshua Pierce's Slack instructions (5 Sept 2026), now Build, Match, Play, Explore, Connect, Get Hired — matched with a career, trying it via Play is the more intuitive next step than Explore. Match's auto-advance after the celebration now goes to Play instead of Explore.
- Landing, Play chapter: removed the small inset of Christina's face beside the dialogue (the main scene already shows her speaking) and the small yellow progress bar above the scene, both flagged as redundant/unpolished.
- Landing, Connect chapter: fixed the real bug — the community board used to auto-scroll to Get Hired 5.2s after the post thread appeared, which could yank the page away mid-explanation during a live demo. It no longer auto-advances; scrolling to Get Hired is entirely the reader's own call now.
- Landing, Connect chapter: the community board card now shows real company logos (Morgan Stanley, EY, HSBC + more) instead of just naming the world, so it reads at a glance that real professionals from real companies are in there. Added an HSBC mark (Wikimedia Commons "HSBC logo (2018)", background plate stripped so it masks cleanly) and tuned EY/HSBC's sizes to match Morgan Stanley's weight.
- Profile: "Archive" renamed to "Saved" throughout (the tab, the drawer, every reference to where a de-prioritized career goes) — matches the vocabulary Instagram and TikTok already use for this, since "Archive" read as "hidden after posting," not "kept for later."
- Career Report: the accent color is now the app's own Dreamari blue for every report, regardless of the career's world — the previous per-industry accent put muddy amber/olive tones on Business & Money reports.
- Match deck: swapped in Data Scientist, Fashion Buyer and Game Designer for Airline Pilot, Registered Nurse and Food Scientist, per the exact list from Joshua Pierce's Slack message — added matching Profile career entries (routes, plan, receipts) so all three hand off cleanly to Profile's Top Three tab; no Career Report exists yet for these three (shows "No report yet," same as any career without one).
- Career Report: found and fixed why the last "blue" fix wasn't actually blue — the override redeclared `--primary` in terms of itself (`{ "--primary": "var(--primary)" }`), a cyclic CSS custom-property reference that computes as invalid and silently fell back to plain dark text instead of the accent color. Removed the override entirely; every --primary/--accent-subtle reference in the report now resolves to the app's own ambient Dreamari blue, which is already the default at the root in both themes.
- Match to Profile handoff: a "Welcome to Your Profile" popup now sits between finishing Match and landing on Profile's Top Three tab (from every finish path), with Dreamy's celebration bounce, a confetti burst and the milestone chime on arrival, the heading revealing word by word (Build's ink reveal), and a Continue button that does the actual handoff.
- Match, Top 3 results popup: the arbitrary gradient on just the word "Your" is gone (a small "Match complete" eyebrow label carries the accent instead); the three career cards now use each career's own poster typeface, cascade in with a stagger, and the #1 pick's gold ring breathes; both popups get a soft glow behind them and a brand-gradient primary CTA with a hover shimmer, kept at the site's standard button shape (var(--radius-md), never a pill).
- Match popups: the shared sheet now caps at 90dvh and scrolls inside itself, so nothing can run off screen on short or landscape tablet viewports.
- Match to Profile handoff, reworked for zero friction (direct feedback, 5 Sept 2026): no dismissable "Welcome" modal any more. Save My Top 3 plays the celebration in place on the Top 3 screen (the cards gather behind your #1, Dreamy bursts in with the chime, the heading becomes "Building your profile") and the page itself carries on to Profile about two seconds later. Every way of finishing Match plays the same shot. The gradient CTA is gone too: gradient buttons exist nowhere else in the app, so it's the standard solid brand-blue Button at the standard radius.
- Profile, arriving from Match (?welcome=1 only): the page assembles itself into view (title, then the identity card, then the tab card) with one line, "Welcome to your profile, Jordan," revealing word by word beside a small Dreamy. A normal visit renders at rest.
- Profile: "Start your resume draft" is now "Create your resume" (all three plan entries); the "Plan" tab is now "My Plan" (Joshua Pierce, Slack, 5 Sept 2026).
- Career Report: High School Classes to Take lists class names only, no one-line "why" under each (can't be written accurately at scale). Colleges for the finance reports are now all within NJ/NY: Columbia University (Reach) replaces Wharton, Seton Hall (Target, in state) replaces Indiana, Bergen Community College (Safety) replaces Pace.
- Match to Profile handoff, back to exactly the flow in the notes (direct feedback, 5 Sept 2026): Save My Top 3 opens a "Welcome to Your Profile" popup with Dreamy, and Continue maps to Profile's Top Three tab. The in-place card stack from the previous pass is gone (it hid the other two picks and looked wrong on tablet). Only the popup's visuals changed: Dreamy's celebration bounce and confetti with the chime, the heading revealing word by word, one caption ("Your Top 3 is saved."), the standard blue Continue button.
- Match to Profile handoff, final shape (direct feedback, 5 Sept 2026): Save My Top 3 goes straight to Profile; the profile assembles into view (title, identity card, tab card) and then the "Welcome to Your Profile" popup with Dreamy introduces the page over a light frosted scrim, never a black screen — the assembled profile stays readable behind it. Continue dismisses it (Top Three is already open) and clears the welcome flag so a refresh doesn't replay it. The small inline Dreamy line under the page title is gone; the popup is the welcome.
- Profile welcome popup: the card is now near-solid (the same surface the quick-links menu and search sheet use) instead of glass — the profile was bleeding through it and the text sat on the page behind. The light scrim around it is unchanged, so the profile still shows around the card.
- Hover states, app-wide: nothing underlines on hover any more. Every text link (`.dm-link`) and Build's Skip link shift color only; underline hover decoration is gone everywhere it existed.
- Match, Your Top 3 Matches: rebuilt as a proper reveal. The sheet opens on a confetti burst and chime, two soft glows drift behind it, "Match complete" pops in, the heading inks in word by word, and the three cards flip up in 3D one after another with a light sheen passing over each; the pick you tap to lead with gets a breathing glow ring and a popped check badge (badge fixed so it only ever shows on the chosen card). Card titles use each career's own poster typeface. CTAs stay the standard Button.
- Match, Your Top 3 Matches on tablet: the three cards now sit side by side from 640px up (they used to stack in a single column until desktop), and the sheet caps at 90dvh and scrolls inside itself so it can't overflow a short landscape viewport.
- Profile, Top Three tab on tablet: the three career cards go three-across from 768px (was desktop only) with reserved heights so titles and captions line up across the row.
- Match deck on tablet and desktop: the deck used to stretch to fill all remaining height, so the header and slot chips hugged the top edge while the CTAs floated far below. The deck now has a bounded height (up to 600px, less on short viewports), and header, slots, deck and CTAs are vertically centered as one group. Phone is unchanged (the deck still fills the screen).
- Career Report medians now follow the Dreamari brief exactly: Investment Banking $361,000, Private Equity $250,000, Software Engineer $136,000, Data Scientist $120,000, Fashion Buyer $78,000, Game Designer $98,000 (source lines note the brief alongside the closest BLS occupation).
- Career Report: full reports written for Data Scientist, Fashion Buyer and Game Designer (at a glance, salary, education path, majors, colleges, comparison, next actions, sources, high school classes). Colleges follow the "one Reach, one Target, one Safety" rule from the notes; UVA and Michigan appear nowhere. Investment Banking and Private Equity list exactly Columbia (Reach), Seton Hall (Target), Bergen Community College (Safety).
- QA pass at 375, 768 and 1280 widths across landing (chapter order, Play chapter, Connect logos), Build (/flow Skip), Match (deck, Top 3 sheet), the Welcome popup, Profile Top Three and Career Report: no horizontal overflow, no stacked Top 3 on tablet, no underlined hover states.
- Landing, Get Hired > My Top 3 card (notes, 5 Sept): the industry line under each career title is gone (Business & Money, Tech & Engineering), and so is the FOCUS pill; the lead card reads as chosen by its blue border and a blue rank badge. Caption now reads "Save your top 3 career paths and compare which one fits you best." The yellow "University" subheading, which made the whole card look like it was about university, is folded into the first row as "University duration", one line, same color and weight as Cost and Median salary. The yellow tints on the card are gone; the card's only accent is Dreamari blue.
- Profile > My Plan, the roadmap (Joshua Pierce, Slack, 5 Sept): rebuilt to the spec. The action label (EXPLORE, PLAY, CONNECT, DECIDE, BUILD, PLAN, EXPERIENCE, APPLY) leads every task; the minutes column is gone everywhere (rows, the Do this next card, the counselor printout); each level groups its steps under In app and Out of app. Clicking an in-app task takes the student straight to that feature (Explore, Play, Connect, Top Three, Resume, Glossary). Out-of-app tasks stay on the roadmap without a link, except the counselor meeting, which opens the Career Report the student brings along. Investment Banking and Private Equity carry the exact finance roadmap: Level 1 Next 3 Months, Foundation; Level 2 Next 6 Months, Skills + People; Level 3 Professional Readiness. Other careers keep their own steps, minus minutes, with Join renamed Connect.
- Landing: the zig-zag is back. After the Play/Explore swap, Match and Play were both image-left and Explore and Connect both image-right; Play now sits image-right and Explore image-left, so the chapters alternate Build, Match, Play, Explore, Connect again.
- Landing, Get Hired > My Plan mock: the "10 min" on the first step is gone to match the product.
- Match deck: the swipe gesture hint is back to first-visit-only. The demo setting that replayed it on every reload is off; progress is remembered in the browser, so a returning student is not retaught.
- Build: one Congratulations screen now. The final Build step carries the copy ("Your matches are ready." / "See the careers that fit you."), screen-wide confetti with Dreamy's bursts and the chime, and a Reveal My Matches button that goes straight to the Match deck. The loading beat with the hard-hat Dreamy and the second "matches are ready" screen are gone.
- Landing, Match chapter: the line now reads "Match with careers and schools that fit who you are." The card backs show "Median salary" with the medians the rest of the app uses (Management Analyst $99K, Investment Banking $361K, Private Equity $250K) instead of ranges. Get Hired's My Top 3 mock and the Investment Banking and Private Equity career detail pages carry the same medians ($361,000 and $250,000 a year), so IB reads the same in Match, Get Hired, career details and the Career Report.
- Career Report: sections reordered and renamed: 01 Overview, 02 High School Classes to Take, 03 3 College Majors to Consider, 04 College Pathways, 05 Colleges. The finance reports list six colleges again, all NJ/NY: Columbia and NYU Stern (Reach), Seton Hall and Rutgers (Target), Bergen Community College and Baruch (Safety). The earlier trim to three was a misread; only the out-of-state schools were meant to go.
- Connect, professional profile (as students see it): rebuilt on the dark, premium look with the Replit's clearer structure. The header is the person's own cover photo from the same set the student profile offers, never the employer's colours; the three numbers are Views, Followers and Likes in short form (48.1K, 1.9K, 6.8K); the personal story stays. "Ask Me" (no longer "Ask Me Anything") holds the composer and the questions already answered in one card. "Career posts" is "My Posts". A Communities section shows the boards the professional answers in, using the exact same community card as the Connect home. "About Me" keeps only Education and Can help with. Likes, saves, chips and buttons are Dreamari blue on every profile, whatever the industry.
- Connect, volunteer view: the Volunteer role now opens the profile as students see it first, with a "My dashboard" button for the private side; the dashboard's Back returns to the profile. The dashboard's numbers, chart and activity ring are Dreamari blue, and the company card sits on the same glass as every other panel.
- Profile, Top Three: a compact "Next step" banner under the cards ("Play your #1 Career Simulation to see if it's really your #1." with a Play button) with a slow glow and an X to dismiss. Routes to the Investment Banking simulation for now.
- Profile, Overview: "Do this next" is two official sentences with the button as the verb: Explore "10 Finance Careers and save your Top 3", or Play "Your #1: Day in the Life of an Investment Banker Simulation".
- Profile header: on tablet and desktop the Grade, GPA and Streak tiles lay out horizontally (icon, label, value on one line) instead of stacking; phones keep the stacked tile. Icons stay.
- Connect: the community card moved into its own module so the home and the professional profile render one and the same card.
- QA: Build end to end, landing, Profile (Overview, Top Three, My Plan, Report, Resume), Connect (student, volunteer profile, dashboard), Play and Colleges checked at phone and desktop widths in dark and light mode; no horizontal overflow.
...
- Connect, professional profile (own view): a Cover button in the header corner opens the same cover picker the student profile has (six covers); the choice is remembered per person. Students see no controls.
- Connect, volunteer dashboard: My communities now uses the same community card as the Connect home and the profile (the small rows carried old thumbnails).
- Landing, desktop: every chapter hugs its content (Explore and Connect already did). Viewport-height chapters centred their row below the visible centre, ran off the bottom on short windows, and doubled the gap where Match met Play (112px against 56px elsewhere); the graphic frame also caps to the visible height. Phones keep the one-chapter-per-screen snap unchanged.
- Landing, desktop, settled: every chapter (Build and Connect included) is one visible screen under the nav, the same height for all six, with its row centred in that space, so each section has its own room and none sits low or gets cut. The graphic frame caps to the visible height (about 540px at a 790px-tall window, 680px max) so the Match card keeps its size. Phone snap unchanged.
- Landing footer: the brand is the Dreamari mark and wordmark (the same Wordmark every header uses) instead of a blue dot and the word.
- Play: a "Looking for another career to play?" banner under the featured row bridges to Explore ("Explore more careers and find another simulation to play." with an Explore button), the same premium banner as the Top Three next step, dismissible. Joshua Pierce's Play to Explore note.
...
- Connect, event boards, Resources tab rebuilt: a photo gallery first (one large frame, four small, the last carrying the remaining count) using licensed event photography from Wikimedia Commons (student panels, a business-student office tour, career fairs, conference audiences; attributions in public/images/connect/events/ATTRIBUTION.md), with a full-screen viewer (arrows, thumbnails, Escape). Files are cards with the file's own icon (slides, folder, PDF, reading list, link), the title and one line of meta; a folder opens on tap to list what it holds. Descriptions, source labels and type chips are gone. Morgan Stanley and Goldman Sachs boards got resources and photos too.
- Connect, event tickets: the whole ticket is the tap target (opens the board, or the code sheet when not yet joined); the stub flip and the buttons keep working on their own. Community cards already behaved this way.
- Profile (Joshua Pierce, Slack, 5 Sept): the Next step banner is more dominant at the same size (deeper breathing glow, a light sweeping across, a soft ring pulse on the Play button) and now also sits at the top of My Plan, so Top Three and My Plan give the same answer to "what now". Streak tile reads "12 day streak · Active 142 of 190 days". My Plan's Level 1 opens with "BUILD: Build Profile" already checked, so the plan starts at 1 of 13 and 8%, never 0%. Resume tab is just "Resume Builder" and "Coming soon".
