...
- Connect, event tickets: the stub's lockup is stacked upright (partner logos are never rotated, per every brand guideline), and "Opens after the event" sits in the action row where the button sits on other cards, so all rows line up.
