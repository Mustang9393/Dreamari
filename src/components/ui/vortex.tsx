"use client";
// Aceternity's Vortex background (https://ui.aceternity.com/components/vortex),
// pulled in via `npx shadcn@latest view @aceternity/vortex-demo-2` per direct
// request 21 Sept 2026 ("Refer the preview on the link... Heres code for the
// animated background"). Adapted to this codebase's own conventions rather
// than kept verbatim: no `cn` (a template literal, same as every other
// component in this repo), and `motion` imported from the already-installed
// `framer-motion` instead of adding the separate `motion` package the
// registry item pulls in (they're the same library's React API). The
// particle-flow algorithm itself -- simplex noise driving each particle's
// velocity, drawn as short strokes with a double-pass blur/lighten glow --
...
export const Vortex = (props: VortexProps) => {
  ...
  // Refs, not plain closure variables -- these are mutated from inside
  // requestAnimationFrame callbacks that outlive a single render, which an
  // ordinary `let` in the component body isn't safe for (flagged by
  ...
  useEffect(() => {
    ...
    // A single still frame for prefers-reduced-motion -- the field's own
    // colors and density, no requestAnimationFrame loop.
    if (reduced) {
      if (canvas && ctx) {
        resize(canvas);
        initParticles();
        ...
        for (let i = 0; i < particlePropsLength; i += particlePropCount) {
          ctx.beginPath();
          ctx.fillStyle = `hsla(${particlePropsRef.current[i + 8]},100%,60%,0.6)`;
          ctx.arc(particlePropsRef.current[i], particlePropsRef.current[i + 1], particlePropsRef.current[i + 7] * 1.4, 0, TAU);
          ctx.fill();
        }
      }
      return;
