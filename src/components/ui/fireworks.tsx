"use client";
// animate-ui's Fireworks Background
// (https://animate-ui.com/docs/components/backgrounds/fireworks), pulled in
// via `npx shadcn@latest view @animate-ui/components-backgrounds-fireworks`
// (a read-only registry call, not `add` -- see docs/AI_HANDOFF.md for why).
// Adapted to this codebase's own conventions: no `cn` (a template literal,
// same as every other component in this repo). Otherwise unchanged from the
// original -- a canvas particle sim, continuously launching fireworks from
// the bottom of its container up to a random height and exploding into
...
function createParticle(x: number, y: number, color: string, speed: number, direction: number, gravity: number, friction: number, size: number): ParticleType {
...
function FireworksBackground({
  ...
  return (
    <div ref={containerRef} data-slot="fireworks-background" className={`relative size-full overflow-hidden ${className ?? ""}`} {...props}>
      <canvas {...canvasProps} ref={canvasRef} className={`absolute inset-0 size-full ${canvasProps?.className ?? ""}`} />
    </div>
  );
}
