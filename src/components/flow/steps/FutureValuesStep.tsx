...
export function FutureValuesStep({ values, onToggleValue, onBack, onNext }: FutureValuesStepProps) {
  return (
    ...
    >
      {/* GIF is now its own full-width row above the two-column split, not part of the
          left column — that's what lets the options grid and the priorities panel below
          match heights exactly (items-stretch), instead of the panel spanning the GIF's
          height too. */}
      ...
      <div className="flex w-full flex-col gap-4 lg:flex-row lg:items-stretch">
        ...
        {/* lg:h-full (with the parent row switched to items-stretch) + lg:justify-center
            makes this a real right-side column spanning the full height of the options
            grid beside it, matching AcademicJourneyStep's and WorkStyleStep's own setup
            panels. The inner wrapper (items-start/text-left) is centered as a whole by
            this outer items-center, but keeps every line left-aligned against the others
            instead of each being independently centered. */}
        {values.length > 0 && (
          <div className="flex w-full flex-col items-center gap-1.5 rounded-xl border border-border-subtle bg-surface-tertiary p-4 lg:h-full lg:w-[240px] lg:shrink-0 lg:justify-center dark:border-white/10 dark:bg-white/5">
