// GENERATED PROTOTYPE DATA (2026-09-03). Every catalog career gets the same
// page structure as Carpenter (the blueprint): summary, "imagine" line, four
// quick facts, pay by state, know about / good at / software, a three-rung
// ladder, education. Figures are approximate, drawn from the BLS Occupational
// Outlook Handbook and O*NET as of 2024, rounded, and in some cases aligned to
// the salary the catalog already shows on the poster (Pediatric Surgeon,
// Purchasing Manager, Cardiologist, Public Relations Manager, Veterinarian).
// Replace with production data when it is available; do not treat as sourced.
import type { CareerProfile } from "./profiles";

export const GENERATED_PROFILES: Record<string, CareerProfile> = {
  "asset-management": {
    "slug": "asset-management",
    "title": "Asset Manager",
    "world": "Business & Money",
    "photo": "/images/app/poster-asset-manager.png",
    "summary": "Invests other people's money to grow it over time.",
    "scenario": "Imagine a retirement fund trusting you with $2 billion. Every morning the market opens and your calls are on the line.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$156,100/year"
      },
      {
        "label": "People doing it",
        "value": "837,100"
      },
      {
        "label": "Jobs open each year",
        "value": "75,100"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$118K"
        }
      ],
      "best": [
        {
          "state": "New York",
          "pay": "$205K"
        },
        {
          "state": "Delaware",
          "pay": "$190K"
        },
        {
          "state": "New Jersey",
          "pay": "$178K"
        }
      ]
    },
    "knowAbout": [
      "Stocks, bonds and funds",
      "Economics",
      "Math and statistics",
      "How companies make money",
      "Risk and reward"
    ],
    "goodAt": [
      "Reading financial statements",
      "Staying calm under pressure",
      "Explaining a decision clearly",
      "Spreadsheet modeling",
      "A CFA charter helps"
    ],
    "software": [
      "Microsoft Excel",
      "Bloomberg Terminal",
      "FactSet",
      "Morningstar Direct",
      "Microsoft PowerPoint"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Investment Analyst",
        "pay": "$85K",
        "description": "You research companies and build the models portfolio managers rely on.",
        "whatYouDo": [
          "Research companies",
          "Build models",
          "Track earnings",
          "Write memos"
        ],
        "toGetHere": [
          "Bachelor's degree",
          "Internship in finance"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Portfolio Associate",
        "pay": "$130K",
        "description": "You cover a sector and pitch your own ideas into the portfolio.",
        "whatYouDo": [
          "Pitch ideas",
          "Monitor holdings",
          "Meet company executives",
          "Manage risk"
        ],
        "toGetHere": [
          "3 to 5 years as analyst",
          "CFA Level II or higher"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Portfolio Manager",
        "pay": "$200K",
        "description": "You own a portfolio and are accountable for its returns.",
        "whatYouDo": [
          "Decide what to buy and sell",
          "Set strategy",
          "Report to clients",
          "Lead a team"
        ],
        "toGetHere": [
          "Track record of results",
          "CFA charter"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Finance"
        },
        {
          "name": "Economics"
        },
        {
          "name": "Business Administration"
        }
      ],
      "where": [
        {
          "count": "1,120",
          "credential": "Bachelor's degree"
        },
        {
          "count": "640",
          "credential": "Master's degree"
        },
        {
          "count": "90",
          "credential": "Doctorate"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "accountant": {
    "slug": "accountant",
    "title": "Accountant",
    "world": "Business & Money",
    "photo": "/images/app/poster-accountant.png",
    "summary": "Keeps the money records straight and makes sure taxes and reports are right.",
    "scenario": "Imagine it is April 10th and a company's tax return is due in five days. Every number has to tie out.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$79,880/year"
      },
      {
        "label": "People doing it",
        "value": "1,562,000"
      },
      {
        "label": "Jobs open each year",
        "value": "130,800"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$68K"
        }
      ],
      "best": [
        {
          "state": "District of Columbia",
          "pay": "$110K"
        },
        {
          "state": "New York",
          "pay": "$105K"
        },
        {
          "state": "New Jersey",
          "pay": "$99K"
        }
      ]
    },
    "knowAbout": [
      "Math",
      "Business",
      "Tax rules",
      "How money moves through a company",
      "Spreadsheets"
    ],
    "goodAt": [
      "Attention to detail",
      "Organizing lots of information",
      "Meeting deadlines",
      "Explaining numbers to non-experts",
      "A CPA license helps"
    ],
    "software": [
      "Microsoft Excel",
      "QuickBooks",
      "SAP",
      "Oracle NetSuite",
      "Intuit ProConnect"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Staff Accountant",
        "pay": "$58K",
        "description": "You record transactions and reconcile accounts under a senior accountant.",
        "whatYouDo": [
          "Post journal entries",
          "Reconcile accounts",
          "Prepare schedules",
          "Help close the month"
        ],
        "toGetHere": [
          "Bachelor's degree",
          "Accounting internship"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Senior Accountant",
        "pay": "$78K",
        "description": "You own the month-end close and review the staff's work.",
        "whatYouDo": [
          "Close the books",
          "Review entries",
          "Prepare reports",
          "Handle audits"
        ],
        "toGetHere": [
          "2 to 4 years experience",
          "CPA exam passed"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Accounting Manager",
        "pay": "$110K",
        "description": "You run the accounting team and own the numbers leadership sees.",
        "whatYouDo": [
          "Lead the team",
          "Set controls",
          "Present to leadership",
          "Plan taxes"
        ],
        "toGetHere": [
          "5+ years experience",
          "CPA license"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Accounting"
        },
        {
          "name": "Finance"
        },
        {
          "name": "Business Administration"
        }
      ],
      "where": [
        {
          "count": "980",
          "credential": "Associate's degree"
        },
        {
          "count": "1,410",
          "credential": "Bachelor's degree"
        },
        {
          "count": "520",
          "credential": "Master's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "private-equity": {
    "slug": "private-equity",
    "title": "Private Equity",
    "world": "Business & Money",
    "photo": "/images/app/poster-private-equity.png",
    "summary": "Buys companies, improves them, and sells them for a profit.",
    "scenario": "Imagine your fund buys a struggling chain of 40 stores. Three years from now it has to be worth twice as much.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$155,000/year"
      },
      {
        "label": "People doing it",
        "value": "347,400"
      },
      {
        "label": "Jobs open each year",
        "value": "30,700"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$95K"
        }
      ],
      "best": [
        {
          "state": "New York",
          "pay": "$210K"
        },
        {
          "state": "Connecticut",
          "pay": "$185K"
        },
        {
          "state": "Massachusetts",
          "pay": "$175K"
        }
      ]
    },
    "knowAbout": [
      "How companies are valued",
      "Deals and negotiation",
      "Economics",
      "Accounting",
      "Debt and financing"
    ],
    "goodAt": [
      "Financial modeling",
      "Long hours in stretches",
      "Persuading people",
      "Spotting what a business is worth",
      "Two years of banking first"
    ],
    "software": [
      "Microsoft Excel",
      "Capital IQ",
      "PitchBook",
      "Bloomberg Terminal",
      "Microsoft PowerPoint"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "PE Analyst",
        "pay": "$100K",
        "description": "You screen deals and build the models the senior team pitches from.",
        "whatYouDo": [
          "Screen deals",
          "Build models",
          "Research industries",
          "Prepare materials"
        ],
        "toGetHere": [
          "Bachelor's degree",
          "Banking or consulting experience"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Associate",
        "pay": "$180K",
        "description": "You run point on live deals and manage the analysts.",
        "whatYouDo": [
          "Run due diligence",
          "Manage analysts",
          "Talk to management teams",
          "Structure deals"
        ],
        "toGetHere": [
          "2 years in banking",
          "Strong modeling record"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Vice President",
        "pay": "$275K",
        "description": "You source your own deals and sit on portfolio company boards.",
        "whatYouDo": [
          "Source deals",
          "Negotiate terms",
          "Join boards",
          "Oversee portfolio companies"
        ],
        "toGetHere": [
          "MBA or equivalent",
          "Closed deals as associate"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Finance"
        },
        {
          "name": "Economics"
        },
        {
          "name": "Business Administration"
        }
      ],
      "where": [
        {
          "count": "1,120",
          "credential": "Bachelor's degree"
        },
        {
          "count": "640",
          "credential": "Master's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "investment-banking": {
    "slug": "investment-banking",
    "title": "Investment Banking",
    "world": "Business & Money",
    "photo": "/images/app/poster-investment-banking-v3.png",
    "summary": "Helps companies raise money and buy or sell businesses.",
    "scenario": "Imagine a hospital company wants to build 100 new hospitals but does not have the money. You find the investors and put the deal together.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$101,910/year"
      },
      {
        "label": "People doing it",
        "value": "500,900"
      },
      {
        "label": "Jobs open each year",
        "value": "36,900"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$72K"
        }
      ],
      "best": [
        {
          "state": "New York",
          "pay": "$150K"
        },
        {
          "state": "Connecticut",
          "pay": "$120K"
        },
        {
          "state": "Massachusetts",
          "pay": "$105K"
        }
      ]
    },
    "knowAbout": [
      "Finance and accounting",
      "Economics",
      "How deals work",
      "Public speaking",
      "Spreadsheets"
    ],
    "goodAt": [
      "Excel modeling",
      "Working long hours in stretches",
      "Precision under pressure",
      "Presenting to executives",
      "A finance or econ degree"
    ],
    "software": [
      "Microsoft Excel",
      "Microsoft PowerPoint",
      "Bloomberg Terminal",
      "FactSet",
      "Capital IQ"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Analyst",
        "pay": "$150K",
        "description": "You crunch the numbers on big deals and build the pitch decks.",
        "whatYouDo": [
          "Build models",
          "Draft pitch decks",
          "Research companies",
          "Support deal teams"
        ],
        "toGetHere": [
          "Bachelor's degree",
          "Summer analyst internship"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Associate",
        "pay": "$225K",
        "description": "You lead small teams and start talking to clients yourself.",
        "whatYouDo": [
          "Manage analysts",
          "Talk to clients",
          "Run deal processes",
          "Check every number"
        ],
        "toGetHere": [
          "2 to 3 years as analyst or an MBA"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Vice President",
        "pay": "$350K",
        "description": "You bring in new business and manage big client relationships.",
        "whatYouDo": [
          "Win mandates",
          "Lead client meetings",
          "Negotiate terms",
          "Mentor juniors"
        ],
        "toGetHere": [
          "3+ years as associate",
          "Track record on deals"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Finance"
        },
        {
          "name": "Economics"
        },
        {
          "name": "Accounting"
        }
      ],
      "where": [
        {
          "count": "1,120",
          "credential": "Bachelor's degree"
        },
        {
          "count": "640",
          "credential": "Master's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "data-scientist": {
    "slug": "data-scientist",
    "title": "Data Scientist",
    "world": "Tech & Engineering",
    "photo": "/images/app/poster-data-scientist.png",
    "summary": "Turns piles of data into answers companies act on.",
    "scenario": "Imagine 40 million app users and one question from the CEO: why did they stop opening it last week?",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$108,020/year"
      },
      {
        "label": "People doing it",
        "value": "202,900"
      },
      {
        "label": "Jobs open each year",
        "value": "20,800"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$82K"
        }
      ],
      "best": [
        {
          "state": "Washington",
          "pay": "$140K"
        },
        {
          "state": "California",
          "pay": "$136K"
        },
        {
          "state": "New York",
          "pay": "$125K"
        }
      ]
    },
    "knowAbout": [
      "Statistics",
      "Programming",
      "Machine learning",
      "How businesses make decisions",
      "Data visualization"
    ],
    "goodAt": [
      "Python or R",
      "Asking the right question",
      "Explaining results simply",
      "SQL",
      "Patience with messy data"
    ],
    "software": [
      "Python",
      "SQL",
      "Jupyter",
      "Tableau",
      "Git / GitHub"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Data Analyst",
        "pay": "$72K",
        "description": "You pull data, build dashboards and answer questions for one team.",
        "whatYouDo": [
          "Write SQL",
          "Build dashboards",
          "Clean data",
          "Report findings"
        ],
        "toGetHere": [
          "Bachelor's degree",
          "Portfolio of projects"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Data Scientist",
        "pay": "$108K",
        "description": "You build models that predict what happens next and test them in production.",
        "whatYouDo": [
          "Build models",
          "Run experiments",
          "Present to product teams",
          "Ship to production"
        ],
        "toGetHere": [
          "2 to 3 years as analyst",
          "Strong statistics"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Senior Data Scientist",
        "pay": "$150K",
        "description": "You set the analytics direction for a product area and mentor others.",
        "whatYouDo": [
          "Lead projects",
          "Mentor",
          "Set standards",
          "Partner with leadership"
        ],
        "toGetHere": [
          "5+ years experience",
          "Master's helps"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Data Science"
        },
        {
          "name": "Statistics"
        },
        {
          "name": "Computer Science"
        }
      ],
      "where": [
        {
          "count": "410",
          "credential": "Bachelor's degree"
        },
        {
          "count": "620",
          "credential": "Master's degree"
        },
        {
          "count": "130",
          "credential": "Doctorate"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "software-engineer": {
    "slug": "software-engineer",
    "title": "Software Engineer",
    "world": "Tech & Engineering",
    "photo": "/images/app/poster-software-engineer.png",
    "summary": "Builds and improves the apps and systems people use every day.",
    "scenario": "Imagine your code shipping to 10 million phones tonight. One bug and every one of them sees it.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$132,270/year"
      },
      {
        "label": "People doing it",
        "value": "1,656,900"
      },
      {
        "label": "Jobs open each year",
        "value": "140,100"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$92K"
        }
      ],
      "best": [
        {
          "state": "California",
          "pay": "$175K"
        },
        {
          "state": "Washington",
          "pay": "$170K"
        },
        {
          "state": "Maryland",
          "pay": "$140K"
        }
      ]
    },
    "knowAbout": [
      "Programming",
      "Math and logic",
      "How computers and networks work",
      "Product design",
      "Teamwork on shared code"
    ],
    "goodAt": [
      "Writing clear code",
      "Debugging",
      "Breaking big problems into small ones",
      "Code review",
      "Learning new tools fast"
    ],
    "software": [
      "VS Code",
      "Git / GitHub",
      "Docker",
      "Jira",
      "Slack"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Junior Engineer",
        "pay": "$85K",
        "description": "You ship small, well-scoped features with a mentor reviewing your code.",
        "whatYouDo": [
          "Write code",
          "Fix bugs",
          "Write tests",
          "Learn the codebase"
        ],
        "toGetHere": [
          "Bachelor's degree or bootcamp",
          "Internship or portfolio"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Software Engineer",
        "pay": "$115K",
        "description": "You own features end to end and review others' code.",
        "whatYouDo": [
          "Design features",
          "Review code",
          "Own releases",
          "Work with product"
        ],
        "toGetHere": [
          "2 to 3 years experience"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Senior Engineer",
        "pay": "$150K",
        "description": "You design systems, not just features, and unblock the team.",
        "whatYouDo": [
          "Design systems",
          "Mentor",
          "Lead projects",
          "Set standards"
        ],
        "toGetHere": [
          "5+ years experience",
          "Track record of shipped systems"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Computer Science"
        },
        {
          "name": "Software Engineering"
        },
        {
          "name": "Computer Engineering"
        }
      ],
      "where": [
        {
          "count": "1,210",
          "credential": "Bachelor's degree"
        },
        {
          "count": "560",
          "credential": "Master's degree"
        },
        {
          "count": "140",
          "credential": "Doctorate"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "ui-ux-designer": {
    "slug": "ui-ux-designer",
    "title": "UI/UX Designer",
    "world": "Tech & Engineering",
    "photo": "/images/app/poster-uiux-designer.png",
    "summary": "Designs how apps look and how they feel to use.",
    "scenario": "Imagine watching five strangers try your checkout screen and every one of them taps the wrong button.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$98,540/year"
      },
      {
        "label": "People doing it",
        "value": "110,000"
      },
      {
        "label": "Jobs open each year",
        "value": "17,000"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$70K"
        }
      ],
      "best": [
        {
          "state": "California",
          "pay": "$130K"
        },
        {
          "state": "Washington",
          "pay": "$125K"
        },
        {
          "state": "New York",
          "pay": "$115K"
        }
      ]
    },
    "knowAbout": [
      "Design and layout",
      "How people think",
      "Color and type",
      "Prototyping",
      "How apps get built"
    ],
    "goodAt": [
      "Sketching ideas fast",
      "Taking feedback",
      "Explaining design decisions",
      "Attention to detail",
      "Figma"
    ],
    "software": [
      "Figma",
      "Adobe Creative Cloud",
      "Notion",
      "Miro",
      "Slack"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Junior Designer",
        "pay": "$62K",
        "description": "You design screens and flows inside a larger product with a senior reviewing your work.",
        "whatYouDo": [
          "Design screens",
          "Build prototypes",
          "Join user tests",
          "Fix visual details"
        ],
        "toGetHere": [
          "Portfolio",
          "Degree or bootcamp"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Product Designer",
        "pay": "$98K",
        "description": "You own a feature area from research through shipped design.",
        "whatYouDo": [
          "Run research",
          "Design flows",
          "Work with engineers",
          "Measure results"
        ],
        "toGetHere": [
          "2 to 4 years experience"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Senior Product Designer",
        "pay": "$140K",
        "description": "You set design direction across a product and mentor the team.",
        "whatYouDo": [
          "Set direction",
          "Mentor",
          "Run critiques",
          "Partner with leadership"
        ],
        "toGetHere": [
          "5+ years experience",
          "Shipped products"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Graphic Design"
        },
        {
          "name": "Interaction Design"
        },
        {
          "name": "Human-Computer Interaction"
        }
      ],
      "where": [
        {
          "count": "560",
          "credential": "Bachelor's degree"
        },
        {
          "count": "210",
          "credential": "Master's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "video-game-designer": {
    "slug": "video-game-designer",
    "title": "Video Game Designer",
    "world": "Tech & Engineering",
    "photo": "/images/app/poster-video-game-designer.png",
    "summary": "Designs the rules, levels and moments that make a game fun.",
    "scenario": "Imagine 200 playtesters quitting your level at the same jump. You have a week to fix it before the demo.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$98,000/year"
      },
      {
        "label": "People doing it",
        "value": "42,000"
      },
      {
        "label": "Jobs open each year",
        "value": "4,500"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$66K"
        }
      ],
      "best": [
        {
          "state": "California",
          "pay": "$125K"
        },
        {
          "state": "Washington",
          "pay": "$120K"
        },
        {
          "state": "Texas",
          "pay": "$95K"
        }
      ]
    },
    "knowAbout": [
      "Game mechanics",
      "Storytelling",
      "Psychology of play",
      "Programming basics",
      "Art and sound"
    ],
    "goodAt": [
      "Prototyping quickly",
      "Taking playtest feedback",
      "Writing clear design docs",
      "Teamwork",
      "A portfolio of small games"
    ],
    "software": [
      "Unity",
      "Unreal Engine",
      "Jira",
      "Figma",
      "Slack"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "QA Tester",
        "pay": "$45K",
        "description": "You play builds all day and write up every bug and rough edge you find.",
        "whatYouDo": [
          "Test builds",
          "Report bugs",
          "Reproduce issues",
          "Give feedback"
        ],
        "toGetHere": [
          "High school diploma",
          "Love of games"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Level Designer",
        "pay": "$75K",
        "description": "You build levels and tune the moment-to-moment feel.",
        "whatYouDo": [
          "Build levels",
          "Tune difficulty",
          "Script events",
          "Run playtests"
        ],
        "toGetHere": [
          "Portfolio",
          "1 to 2 years in QA or school projects"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Lead Designer",
        "pay": "$120K",
        "description": "You own the vision for a game's systems and lead the design team.",
        "whatYouDo": [
          "Set the vision",
          "Lead designers",
          "Pitch to publishers",
          "Balance systems"
        ],
        "toGetHere": [
          "5+ years experience",
          "Shipped titles"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Game Design"
        },
        {
          "name": "Computer Science"
        },
        {
          "name": "Interactive Media"
        }
      ],
      "where": [
        {
          "count": "380",
          "credential": "Bachelor's degree"
        },
        {
          "count": "90",
          "credential": "Master's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "cyber-security": {
    "slug": "cyber-security",
    "title": "Cyber Security",
    "world": "Tech & Engineering",
    "photo": "/images/app/poster-cyber-security.png",
    "summary": "Protects computer systems and data from attacks.",
    "scenario": "Imagine an alert at 2 a.m.: someone is inside the network. You have minutes to find them before they find the customer data.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$120,360/year"
      },
      {
        "label": "People doing it",
        "value": "180,700"
      },
      {
        "label": "Jobs open each year",
        "value": "17,300"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$88K"
        }
      ],
      "best": [
        {
          "state": "California",
          "pay": "$150K"
        },
        {
          "state": "New York",
          "pay": "$145K"
        },
        {
          "state": "District of Columbia",
          "pay": "$140K"
        }
      ]
    },
    "knowAbout": [
      "How networks work",
      "Operating systems",
      "How attackers think",
      "Encryption",
      "Laws about data"
    ],
    "goodAt": [
      "Staying calm in an incident",
      "Noticing what does not fit",
      "Scripting",
      "Writing clear reports",
      "Certifications like Security+"
    ],
    "software": [
      "Splunk",
      "Wireshark",
      "Linux",
      "Python",
      "Microsoft Sentinel"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Security Analyst",
        "pay": "$75K",
        "description": "You watch alerts, investigate them and escalate the real ones.",
        "whatYouDo": [
          "Monitor alerts",
          "Investigate incidents",
          "Patch systems",
          "Write reports"
        ],
        "toGetHere": [
          "Bachelor's degree or certification",
          "Security+"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Security Engineer",
        "pay": "$120K",
        "description": "You design the defenses and run the response when something gets through.",
        "whatYouDo": [
          "Design defenses",
          "Run incident response",
          "Test systems",
          "Automate detection"
        ],
        "toGetHere": [
          "3 to 5 years experience",
          "CISSP or similar"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Security Architect",
        "pay": "$165K",
        "description": "You set the security strategy for the whole company.",
        "whatYouDo": [
          "Set strategy",
          "Review architecture",
          "Advise leadership",
          "Lead the team"
        ],
        "toGetHere": [
          "8+ years experience",
          "Advanced certifications"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Cybersecurity"
        },
        {
          "name": "Computer Science"
        },
        {
          "name": "Information Technology"
        }
      ],
      "where": [
        {
          "count": "320",
          "credential": "Associate's degree"
        },
        {
          "count": "690",
          "credential": "Bachelor's degree"
        },
        {
          "count": "340",
          "credential": "Master's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "food-scientist": {
    "slug": "food-scientist",
    "title": "Food Scientist",
    "world": "Farming, Animals & Nature",
    "photo": "/images/app/poster-food-scientist.png",
    "summary": "Figures out how to make food safer, tastier and last longer.",
    "scenario": "Imagine a new snack that tastes perfect in the lab but goes stale in 9 days on the shelf. You have to get it to 90.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$76,400/year"
      },
      {
        "label": "People doing it",
        "value": "40,600"
      },
      {
        "label": "Jobs open each year",
        "value": "3,200"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$62K"
        }
      ],
      "best": [
        {
          "state": "Maryland",
          "pay": "$105K"
        },
        {
          "state": "California",
          "pay": "$96K"
        },
        {
          "state": "Illinois",
          "pay": "$90K"
        }
      ]
    },
    "knowAbout": [
      "Chemistry and biology",
      "Nutrition",
      "Food safety rules",
      "Statistics",
      "How food is manufactured"
    ],
    "goodAt": [
      "Careful lab work",
      "Keeping precise records",
      "Tasting and describing flavor",
      "Problem solving",
      "Working with factory teams"
    ],
    "software": [
      "Microsoft Excel",
      "LIMS",
      "Minitab",
      "Genesis R&D",
      "SAP"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Lab Technician",
        "pay": "$42K",
        "description": "You run tests and keep the lab's data clean and organized.",
        "whatYouDo": [
          "Run tests",
          "Prepare samples",
          "Log data",
          "Maintain equipment"
        ],
        "toGetHere": [
          "Associate's or bachelor's degree"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Food Scientist",
        "pay": "$76K",
        "description": "You develop and test new products from concept through shelf.",
        "whatYouDo": [
          "Develop formulas",
          "Run shelf-life tests",
          "Lead sensory panels",
          "Scale to production"
        ],
        "toGetHere": [
          "Bachelor's degree",
          "1 to 3 years in a lab"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Senior Food Scientist",
        "pay": "$105K",
        "description": "You lead formulation projects and mentor junior scientists.",
        "whatYouDo": [
          "Lead projects",
          "Mentor",
          "Own product lines",
          "Work with regulators"
        ],
        "toGetHere": [
          "5+ years experience",
          "Master's helps"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Food Science"
        },
        {
          "name": "Chemistry"
        },
        {
          "name": "Nutrition"
        }
      ],
      "where": [
        {
          "count": "210",
          "credential": "Bachelor's degree"
        },
        {
          "count": "140",
          "credential": "Master's degree"
        },
        {
          "count": "60",
          "credential": "Doctorate"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "farm-and-ranch-manager": {
    "slug": "farm-and-ranch-manager",
    "title": "Farm & Ranch Manager",
    "world": "Farming, Animals & Nature",
    "photo": "/images/app/poster-farm-ranch-manager.png",
    "summary": "Runs the land, animals and crews that produce our food.",
    "scenario": "Imagine a frost warning at 11 p.m. and 400 acres of young corn. What you decide tonight is the whole year's income.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "High school diploma or equivalent"
      },
      {
        "label": "Typical pay",
        "value": "$83,770/year"
      },
      {
        "label": "People doing it",
        "value": "838,200"
      },
      {
        "label": "Jobs open each year",
        "value": "88,600"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$79K"
        }
      ],
      "best": [
        {
          "state": "California",
          "pay": "$120K"
        },
        {
          "state": "Washington",
          "pay": "$110K"
        },
        {
          "state": "Nebraska",
          "pay": "$100K"
        }
      ]
    },
    "knowAbout": [
      "Plants, animals and soil",
      "Weather and seasons",
      "Machinery",
      "Business and budgets",
      "Markets for crops and livestock"
    ],
    "goodAt": [
      "Long days outdoors",
      "Fixing things yourself",
      "Planning months ahead",
      "Managing a crew",
      "Handling risk"
    ],
    "software": [
      "John Deere Operations Center",
      "Microsoft Excel",
      "QuickBooks",
      "Climate FieldView",
      "Granular"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Farmhand",
        "pay": "$36K",
        "description": "You do the daily work: feeding, planting, fixing, hauling.",
        "whatYouDo": [
          "Feed and water animals",
          "Plant and harvest",
          "Run equipment",
          "Repair fences"
        ],
        "toGetHere": [
          "High school diploma",
          "Willingness to work outdoors"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Assistant Manager",
        "pay": "$55K",
        "description": "You run a crew and a piece of the operation on your own.",
        "whatYouDo": [
          "Lead a crew",
          "Schedule work",
          "Track inputs and costs",
          "Handle problems"
        ],
        "toGetHere": [
          "3 to 5 years on farms",
          "Equipment skills"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Farm or Ranch Manager",
        "pay": "$84K",
        "description": "You run the whole operation and its money.",
        "whatYouDo": [
          "Plan the season",
          "Buy and sell",
          "Manage budgets",
          "Hire and lead"
        ],
        "toGetHere": [
          "Years of experience",
          "Business training helps"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Agricultural Business"
        },
        {
          "name": "Animal Science"
        },
        {
          "name": "Agronomy"
        }
      ],
      "where": [
        {
          "count": "330",
          "credential": "Associate's degree"
        },
        {
          "count": "290",
          "credential": "Bachelor's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "agricultural-technician": {
    "slug": "agricultural-technician",
    "title": "Agricultural Technician",
    "world": "Farming, Animals & Nature",
    "photo": "/images/app/poster-agricultural-technician.png",
    "summary": "Runs the tests and field trials that make farms and food better.",
    "scenario": "Imagine 300 soil samples on your bench by Friday, and a grower waiting on the results to decide what to plant.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Associate's degree"
      },
      {
        "label": "Typical pay",
        "value": "$45,880/year"
      },
      {
        "label": "People doing it",
        "value": "33,900"
      },
      {
        "label": "Jobs open each year",
        "value": "4,100"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$41K"
        }
      ],
      "best": [
        {
          "state": "Alaska",
          "pay": "$60K"
        },
        {
          "state": "Washington",
          "pay": "$58K"
        },
        {
          "state": "California",
          "pay": "$55K"
        }
      ]
    },
    "knowAbout": [
      "Biology and chemistry",
      "Plants and animals",
      "Lab procedures",
      "Data and record keeping",
      "Farm equipment"
    ],
    "goodAt": [
      "Careful, repeatable work",
      "Working outdoors and in the lab",
      "Following procedures exactly",
      "Basic math",
      "Noticing small changes"
    ],
    "software": [
      "Microsoft Excel",
      "LIMS",
      "ArcGIS",
      "Microsoft Word",
      "Field data loggers"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Field Technician",
        "pay": "$36K",
        "description": "You collect samples and set up trials under a scientist's direction.",
        "whatYouDo": [
          "Collect samples",
          "Set up plots",
          "Record data",
          "Maintain equipment"
        ],
        "toGetHere": [
          "High school diploma",
          "Some college helps"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Agricultural Technician",
        "pay": "$46K",
        "description": "You run lab tests and field trials and keep the data clean.",
        "whatYouDo": [
          "Run tests",
          "Manage trials",
          "Analyze results",
          "Write reports"
        ],
        "toGetHere": [
          "Associate's degree",
          "1 to 2 years in the field"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Senior Technician",
        "pay": "$60K",
        "description": "You supervise technicians and run the lab or field program day to day.",
        "whatYouDo": [
          "Supervise",
          "Train new staff",
          "Plan trials",
          "Report to scientists"
        ],
        "toGetHere": [
          "5+ years experience",
          "Bachelor's helps"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Agricultural Science"
        },
        {
          "name": "Biology"
        },
        {
          "name": "Environmental Science"
        }
      ],
      "where": [
        {
          "count": "260",
          "credential": "Associate's degree"
        },
        {
          "count": "310",
          "credential": "Bachelor's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "electrician": {
    "slug": "electrician",
    "title": "Electrician",
    "world": "Building & Construction",
    "photo": "/images/app/poster-electrician.png",
    "summary": "Installs and fixes the wiring that powers buildings.",
    "scenario": "Imagine a hospital wing going live Monday and 2,000 connections that all have to be right. One loose wire and the lights go out in surgery.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "High school diploma or equivalent"
      },
      {
        "label": "Typical pay",
        "value": "$61,590/year"
      },
      {
        "label": "People doing it",
        "value": "762,600"
      },
      {
        "label": "Jobs open each year",
        "value": "80,200"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$56K"
        }
      ],
      "best": [
        {
          "state": "Illinois",
          "pay": "$85K"
        },
        {
          "state": "Alaska",
          "pay": "$82K"
        },
        {
          "state": "Hawaii",
          "pay": "$80K"
        }
      ]
    },
    "knowAbout": [
      "Electricity and circuits",
      "Building codes",
      "Math",
      "Reading blueprints",
      "Safety rules"
    ],
    "goodAt": [
      "Working with your hands",
      "Precision",
      "Troubleshooting",
      "Physical stamina",
      "A 4 to 5 year apprenticeship"
    ],
    "software": [
      "Microsoft Excel",
      "AutoCAD Electrical",
      "Procore",
      "Bluebeam",
      "QuickBooks"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Apprentice Electrician",
        "pay": "$40K",
        "description": "You learn on the job alongside a licensed electrician while taking classes.",
        "whatYouDo": [
          "Pull wire",
          "Install boxes",
          "Learn the code",
          "Assist journeymen"
        ],
        "toGetHere": [
          "High school diploma",
          "Accepted into apprenticeship"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Journeyman Electrician",
        "pay": "$62K",
        "description": "You wire buildings on your own and read the plans yourself.",
        "whatYouDo": [
          "Wire buildings",
          "Troubleshoot",
          "Read plans",
          "Pass inspections"
        ],
        "toGetHere": [
          "4 to 5 year apprenticeship",
          "State license"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Master Electrician",
        "pay": "$85K",
        "description": "You run jobs, pull permits and can own your own shop.",
        "whatYouDo": [
          "Run jobs",
          "Pull permits",
          "Bid work",
          "Lead crews"
        ],
        "toGetHere": [
          "Years as journeyman",
          "Master license"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Electrician"
        },
        {
          "name": "Electrical Technology"
        }
      ],
      "where": [
        {
          "count": "410",
          "credential": "Certificate, under a year"
        },
        {
          "count": "380",
          "credential": "Certificate, 1-2 years"
        },
        {
          "count": "120",
          "credential": "Associate's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "roofer": {
    "slug": "roofer",
    "title": "Roofer",
    "world": "Building & Construction",
    "photo": "/images/app/poster-roofer.png",
    "summary": "Installs and repairs the roofs that keep buildings dry.",
    "scenario": "Imagine a storm coming Thursday and a school roof half torn off. Your crew has three days.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "No formal education required"
      },
      {
        "label": "Typical pay",
        "value": "$50,030/year"
      },
      {
        "label": "People doing it",
        "value": "149,000"
      },
      {
        "label": "Jobs open each year",
        "value": "12,100"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$46K"
        }
      ],
      "best": [
        {
          "state": "Illinois",
          "pay": "$70K"
        },
        {
          "state": "Washington",
          "pay": "$68K"
        },
        {
          "state": "Minnesota",
          "pay": "$65K"
        }
      ]
    },
    "knowAbout": [
      "Building materials",
      "Measuring and math",
      "Weather",
      "Safety rules",
      "Reading plans"
    ],
    "goodAt": [
      "Working at heights",
      "Physical strength",
      "Working in heat and cold",
      "Careful, steady work",
      "Teamwork on a crew"
    ],
    "software": [
      "Microsoft Excel",
      "EagleView",
      "AccuLynx",
      "JobNimbus",
      "QuickBooks"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Roofing Laborer",
        "pay": "$36K",
        "description": "You carry materials, tear off old roofs and learn the trade.",
        "whatYouDo": [
          "Tear off",
          "Carry materials",
          "Clean up",
          "Learn safety"
        ],
        "toGetHere": [
          "No degree required",
          "Comfortable at heights"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Roofer",
        "pay": "$50K",
        "description": "You install shingles, membranes and flashing on your own.",
        "whatYouDo": [
          "Install roofing",
          "Seal and flash",
          "Repair leaks",
          "Read plans"
        ],
        "toGetHere": [
          "1 to 3 years on crews"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Roofing Foreman",
        "pay": "$68K",
        "description": "You run the crew and the job site.",
        "whatYouDo": [
          "Run crews",
          "Plan jobs",
          "Meet inspectors",
          "Order materials"
        ],
        "toGetHere": [
          "Years of experience",
          "Safety certification"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Construction Trades"
        }
      ],
      "where": [
        {
          "count": "140",
          "credential": "Certificate, under a year"
        },
        {
          "count": "60",
          "credential": "Certificate, 1-2 years"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "truck-driver": {
    "slug": "truck-driver",
    "title": "Truck Driver",
    "world": "Driving, Flying & Shipping",
    "photo": "/images/app/poster-truck-driver.png",
    "summary": "Moves the goods that fill every store shelf across the country.",
    "scenario": "Imagine 40,000 pounds behind you, 600 miles to go, and a delivery window that closes at 6 a.m.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "High school diploma or equivalent"
      },
      {
        "label": "Typical pay",
        "value": "$54,320/year"
      },
      {
        "label": "People doing it",
        "value": "2,200,000"
      },
      {
        "label": "Jobs open each year",
        "value": "241,200"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$51K"
        }
      ],
      "best": [
        {
          "state": "Alaska",
          "pay": "$70K"
        },
        {
          "state": "District of Columbia",
          "pay": "$68K"
        },
        {
          "state": "New York",
          "pay": "$64K"
        }
      ]
    },
    "knowAbout": [
      "Road rules and safety",
      "Vehicle mechanics",
      "Maps and routes",
      "Logbooks and regulations",
      "Time management"
    ],
    "goodAt": [
      "Staying alert for hours",
      "Patience",
      "Handling a big vehicle",
      "Independence",
      "A commercial driver's license"
    ],
    "software": [
      "Electronic logging devices",
      "Samsara",
      "Trimble",
      "Google Maps",
      "Microsoft Excel"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "CDL Student",
        "pay": "",
        "description": "You train for and pass the commercial driver's license.",
        "whatYouDo": [
          "Learn the rules",
          "Practice driving",
          "Pass the CDL exam",
          "Learn inspections"
        ],
        "toGetHere": [
          "High school diploma",
          "Age 21 for interstate"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Company Driver",
        "pay": "$54K",
        "description": "You haul freight for a carrier on set routes or over the road.",
        "whatYouDo": [
          "Drive routes",
          "Inspect the truck",
          "Keep logs",
          "Deliver on time"
        ],
        "toGetHere": [
          "CDL",
          "Clean driving record"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Owner-Operator",
        "pay": "$80K",
        "description": "You own your truck and run your own business.",
        "whatYouDo": [
          "Find loads",
          "Manage costs",
          "Maintain the truck",
          "Run the business"
        ],
        "toGetHere": [
          "Years of driving",
          "Money to buy a truck"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Truck and Bus Driver / Commercial Vehicle Operator"
        }
      ],
      "where": [
        {
          "count": "620",
          "credential": "Certificate, under a year"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "registered-nurse": {
    "slug": "registered-nurse",
    "title": "Registered Nurse",
    "world": "Health & Medicine",
    "photo": "/images/app/poster-registered-nurse.png",
    "summary": "Keeps patients safe and is the person actually at the bedside all shift.",
    "scenario": "Imagine six patients, one code blue, and a family asking you what happens next. All at 3 a.m.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$86,070/year"
      },
      {
        "label": "People doing it",
        "value": "3,300,100"
      },
      {
        "label": "Jobs open each year",
        "value": "193,100"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$65K"
        }
      ],
      "best": [
        {
          "state": "California",
          "pay": "$137K"
        },
        {
          "state": "Hawaii",
          "pay": "$119K"
        },
        {
          "state": "Oregon",
          "pay": "$110K"
        }
      ]
    },
    "knowAbout": [
      "Anatomy and biology",
      "Medications",
      "How hospitals work",
      "Talking with patients and families",
      "Charting"
    ],
    "goodAt": [
      "Staying calm under pressure",
      "Noticing small changes",
      "Long shifts on your feet",
      "Clear communication",
      "Passing the NCLEX"
    ],
    "software": [
      "Epic (EHR)",
      "Cerner",
      "Pyxis",
      "Microsoft Excel",
      "Hospital scheduling software"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Nursing Student / CNA",
        "pay": "$35K",
        "description": "You get real patient contact hours before you are even licensed.",
        "whatYouDo": [
          "Basic patient care",
          "Take vitals",
          "Clinical rotations",
          "Learn charting"
        ],
        "toGetHere": [
          "Enrolled in a nursing program",
          "CNA certificate"
        ]
      },
      {
        "number": "2",
        "jobTitle": "New Grad RN",
        "pay": "$65K",
        "description": "Your first licensed job, usually on a structured residency unit.",
        "whatYouDo": [
          "Give medications",
          "Chart",
          "Assess patients",
          "Work with the team"
        ],
        "toGetHere": [
          "BSN or ADN",
          "NCLEX passed"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Charge Nurse",
        "pay": "$95K",
        "description": "You run the floor for your shift and support the newer nurses.",
        "whatYouDo": [
          "Assign patients",
          "Lead the shift",
          "Handle escalations",
          "Mentor"
        ],
        "toGetHere": [
          "3 to 5 years experience",
          "Unit leadership training"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Nursing (BSN)"
        },
        {
          "name": "Nursing (ADN)"
        }
      ],
      "where": [
        {
          "count": "1,090",
          "credential": "Associate's degree"
        },
        {
          "count": "890",
          "credential": "Bachelor's degree"
        },
        {
          "count": "560",
          "credential": "Master's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "school-counselor": {
    "slug": "school-counselor",
    "title": "School Counselor",
    "world": "Counseling & Social Work",
    "photo": "/images/app/poster-school-counselor.png",
    "summary": "Helps students through school, college plans and hard days.",
    "scenario": "Imagine a junior in your office who just said they are not going to college because nobody in their family did. You have 20 minutes.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Master's degree"
      },
      {
        "label": "Typical pay",
        "value": "$61,710/year"
      },
      {
        "label": "People doing it",
        "value": "371,900"
      },
      {
        "label": "Jobs open each year",
        "value": "26,600"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$50K"
        }
      ],
      "best": [
        {
          "state": "California",
          "pay": "$87K"
        },
        {
          "state": "Washington",
          "pay": "$82K"
        },
        {
          "state": "New Jersey",
          "pay": "$80K"
        }
      ]
    },
    "knowAbout": [
      "How teens think and grow",
      "College and career paths",
      "Mental health basics",
      "School systems",
      "Listening"
    ],
    "goodAt": [
      "Patience",
      "Keeping confidences",
      "Explaining options simply",
      "Staying organized across hundreds of students",
      "A state license"
    ],
    "software": [
      "Naviance",
      "PowerSchool",
      "Google Workspace",
      "Microsoft Excel",
      "Zoom"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Counseling Intern",
        "pay": "",
        "description": "You counsel students under a licensed counselor while finishing your master's.",
        "whatYouDo": [
          "Meet students",
          "Run groups",
          "Learn systems",
          "Shadow"
        ],
        "toGetHere": [
          "Enrolled in a counseling master's"
        ]
      },
      {
        "number": "2",
        "jobTitle": "School Counselor",
        "pay": "$62K",
        "description": "You carry a caseload of students and their plans.",
        "whatYouDo": [
          "Meet students",
          "Plan schedules",
          "Run college nights",
          "Respond to crises"
        ],
        "toGetHere": [
          "Master's degree",
          "State certification"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Lead Counselor",
        "pay": "$78K",
        "description": "You run the counseling department for a school or district.",
        "whatYouDo": [
          "Lead the team",
          "Set programs",
          "Work with principals",
          "Report outcomes"
        ],
        "toGetHere": [
          "5+ years experience"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "School Counseling"
        },
        {
          "name": "Counseling Psychology"
        },
        {
          "name": "Education"
        }
      ],
      "where": [
        {
          "count": "640",
          "credential": "Master's degree"
        },
        {
          "count": "90",
          "credential": "Doctorate"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "air-traffic-controller": {
    "slug": "air-traffic-controller",
    "title": "Air Traffic Controller",
    "world": "Driving, Flying & Shipping",
    "photo": "/images/app/poster-air-traffic-controller.png",
    "summary": "Keeps planes safely apart in the sky and on the runway.",
    "scenario": "Imagine 14 planes on your screen, a thunderstorm closing one runway, and every pilot waiting for your voice.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Associate's degree"
      },
      {
        "label": "Typical pay",
        "value": "$137,380/year"
      },
      {
        "label": "People doing it",
        "value": "22,700"
      },
      {
        "label": "Jobs open each year",
        "value": "2,200"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$120K"
        }
      ],
      "best": [
        {
          "state": "New Hampshire",
          "pay": "$170K"
        },
        {
          "state": "Illinois",
          "pay": "$165K"
        },
        {
          "state": "Virginia",
          "pay": "$160K"
        }
      ]
    },
    "knowAbout": [
      "Aviation rules",
      "Geography and weather",
      "Radar and radio",
      "Math done fast in your head",
      "Aircraft performance"
    ],
    "goodAt": [
      "Staying calm and precise",
      "Speaking clearly",
      "Holding many things in your head",
      "Quick decisions",
      "Passing the FAA screening"
    ],
    "software": [
      "FAA STARS",
      "ERAM",
      "Radar displays",
      "Flight strips",
      "Microsoft Excel"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Academy Trainee",
        "pay": "$40K",
        "description": "You train at the FAA Academy in Oklahoma City before you ever touch live traffic.",
        "whatYouDo": [
          "Learn procedures",
          "Run simulations",
          "Pass evaluations",
          "Study airspace"
        ],
        "toGetHere": [
          "Age under 31 at hire",
          "Pass FAA screening"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Developmental Controller",
        "pay": "$80K",
        "description": "You work live traffic under a certified controller's supervision.",
        "whatYouDo": [
          "Work positions",
          "Certify position by position",
          "Handle real traffic",
          "Debrief"
        ],
        "toGetHere": [
          "Academy graduate",
          "Medical clearance"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Certified Controller",
        "pay": "$137K",
        "description": "You are fully certified at your facility and run the sky on your own.",
        "whatYouDo": [
          "Separate aircraft",
          "Sequence arrivals",
          "Coordinate facilities",
          "Train developmentals"
        ],
        "toGetHere": [
          "2 to 4 years developmental"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Air Traffic Control"
        },
        {
          "name": "Aviation"
        }
      ],
      "where": [
        {
          "count": "40",
          "credential": "Associate's degree"
        },
        {
          "count": "30",
          "credential": "Bachelor's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "sports-medicine-doctor": {
    "slug": "sports-medicine-doctor",
    "title": "Sports Medicine Doctor",
    "world": "Health & Medicine",
    "photo": "/images/app/poster-sports-medicine-doctor.png",
    "summary": "Treats athletes' injuries and gets them safely back in the game.",
    "scenario": "Imagine a point guard down with a knee injury and a playoff game in nine days. Everyone wants a yes. Your job is the honest answer.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Doctoral or professional degree"
      },
      {
        "label": "Typical pay",
        "value": "$236,000/year"
      },
      {
        "label": "People doing it",
        "value": "25,000"
      },
      {
        "label": "Jobs open each year",
        "value": "1,200"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$210K"
        }
      ],
      "best": [
        {
          "state": "Wisconsin",
          "pay": "$300K"
        },
        {
          "state": "Indiana",
          "pay": "$290K"
        },
        {
          "state": "Georgia",
          "pay": "$280K"
        }
      ]
    },
    "knowAbout": [
      "Anatomy and biology",
      "How the body heals",
      "Exercise science",
      "Imaging",
      "Talking with patients"
    ],
    "goodAt": [
      "Years of study",
      "Steady hands",
      "Saying no to a star player",
      "Teamwork with trainers",
      "A medical license"
    ],
    "software": [
      "Epic (EHR)",
      "PACS imaging software",
      "Microsoft Excel",
      "Dragon Medical",
      "Zoom"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Medical Student",
        "pay": "",
        "description": "Four years of medical school before you see your own patients.",
        "whatYouDo": [
          "Study",
          "Rotate through specialties",
          "Pass boards",
          "Shadow"
        ],
        "toGetHere": [
          "Bachelor's degree",
          "MCAT"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Resident",
        "pay": "$65K",
        "description": "Three years of family or emergency medicine training in a hospital.",
        "whatYouDo": [
          "See patients",
          "Take call",
          "Learn procedures",
          "Present cases"
        ],
        "toGetHere": [
          "Medical degree",
          "Match into residency"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Sports Medicine Physician",
        "pay": "$236K",
        "description": "You finish a fellowship and care for athletes and active patients on your own.",
        "whatYouDo": [
          "Diagnose injuries",
          "Plan recovery",
          "Cover games",
          "Work with trainers"
        ],
        "toGetHere": [
          "Sports medicine fellowship",
          "Board certification"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Biology"
        },
        {
          "name": "Kinesiology"
        },
        {
          "name": "Pre-Medicine"
        }
      ],
      "where": [
        {
          "count": "1,300",
          "credential": "Bachelor's degree"
        },
        {
          "count": "190",
          "credential": "Doctorate"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "animator": {
    "slug": "animator",
    "title": "Animator",
    "world": "Arts, Media & Sport",
    "photo": "/images/app/poster-animator.png",
    "summary": "Brings characters and scenes to life frame by frame.",
    "scenario": "Imagine a 4-second shot of a character laughing. It takes you two weeks, and the audience never thinks about it once. That is the point.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$99,060/year"
      },
      {
        "label": "People doing it",
        "value": "89,300"
      },
      {
        "label": "Jobs open each year",
        "value": "6,700"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$60K"
        }
      ],
      "best": [
        {
          "state": "California",
          "pay": "$130K"
        },
        {
          "state": "Washington",
          "pay": "$120K"
        },
        {
          "state": "New York",
          "pay": "$110K"
        }
      ]
    },
    "knowAbout": [
      "Drawing and design",
      "How things move",
      "Storytelling",
      "Film and cameras",
      "3D software"
    ],
    "goodAt": [
      "Patience for tiny details",
      "Taking notes from directors",
      "Meeting deadlines",
      "Observing how people move",
      "A strong reel"
    ],
    "software": [
      "Autodesk Maya",
      "Blender",
      "Adobe After Effects",
      "Toon Boom Harmony",
      "ZBrush"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Junior Animator",
        "pay": "$55K",
        "description": "You animate simple shots and clean up others' work under a lead.",
        "whatYouDo": [
          "Animate shots",
          "Clean up",
          "Take notes",
          "Learn the pipeline"
        ],
        "toGetHere": [
          "Portfolio reel",
          "Degree or strong self-training"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Animator",
        "pay": "$99K",
        "description": "You own shots and scenes and bring characters to life.",
        "whatYouDo": [
          "Animate scenes",
          "Block and polish",
          "Work with directors",
          "Hit deadlines"
        ],
        "toGetHere": [
          "2 to 4 years experience"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Animation Supervisor",
        "pay": "$140K",
        "description": "You lead the animation team and set the style for a show or film.",
        "whatYouDo": [
          "Lead animators",
          "Set style",
          "Review shots",
          "Plan schedules"
        ],
        "toGetHere": [
          "7+ years experience",
          "Shipped films or shows"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Animation"
        },
        {
          "name": "Fine Arts"
        },
        {
          "name": "Digital Media"
        }
      ],
      "where": [
        {
          "count": "380",
          "credential": "Bachelor's degree"
        },
        {
          "count": "70",
          "credential": "Master's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "hr-manager": {
    "slug": "hr-manager",
    "title": "HR Manager",
    "world": "Business & Money",
    "photo": "/images/app/poster-hr-manager.png",
    "summary": "Hires, supports and looks after the people who make a company run.",
    "scenario": "Imagine 40 open roles, a layoff rumor on Slack, and a new manager who needs coaching by Friday.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$136,350/year"
      },
      {
        "label": "People doing it",
        "value": "200,600"
      },
      {
        "label": "Jobs open each year",
        "value": "15,600"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$96K"
        }
      ],
      "best": [
        {
          "state": "New York",
          "pay": "$185K"
        },
        {
          "state": "New Jersey",
          "pay": "$175K"
        },
        {
          "state": "District of Columbia",
          "pay": "$170K"
        }
      ]
    },
    "knowAbout": [
      "Employment law",
      "How people work in teams",
      "Pay and benefits",
      "Business strategy",
      "Communication"
    ],
    "goodAt": [
      "Listening",
      "Handling conflict calmly",
      "Keeping confidences",
      "Organizing many moving parts",
      "An HR certification helps"
    ],
    "software": [
      "Workday",
      "LinkedIn Recruiter",
      "Greenhouse",
      "Microsoft Excel",
      "Slack"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "HR Coordinator",
        "pay": "$52K",
        "description": "You keep hiring, onboarding and records moving day to day.",
        "whatYouDo": [
          "Schedule interviews",
          "Onboard new hires",
          "Maintain records",
          "Answer questions"
        ],
        "toGetHere": [
          "Bachelor's degree"
        ]
      },
      {
        "number": "2",
        "jobTitle": "HR Generalist",
        "pay": "$75K",
        "description": "You handle benefits, employee issues and policies for a group.",
        "whatYouDo": [
          "Support managers",
          "Run benefits",
          "Resolve issues",
          "Improve policies"
        ],
        "toGetHere": [
          "2 to 4 years experience",
          "SHRM-CP helps"
        ]
      },
      {
        "number": "3",
        "jobTitle": "HR Manager",
        "pay": "$136K",
        "description": "You run HR for a business unit or company.",
        "whatYouDo": [
          "Set people strategy",
          "Lead the team",
          "Advise executives",
          "Own compliance"
        ],
        "toGetHere": [
          "7+ years experience",
          "SHRM-SCP or SPHR"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Human Resources Management"
        },
        {
          "name": "Business Administration"
        },
        {
          "name": "Psychology"
        }
      ],
      "where": [
        {
          "count": "510",
          "credential": "Bachelor's degree"
        },
        {
          "count": "270",
          "credential": "Master's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "game-designer": {
    "slug": "game-designer",
    "title": "Game Designer",
    "world": "Tech & Engineering",
    "photo": "/images/app/poster-game-designer.png",
    "summary": "Designs the systems and rules that make a game worth playing.",
    "scenario": "Imagine a game economy where players found a trick that prints gold. Ten thousand of them are using it by lunch.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$98,000/year"
      },
      {
        "label": "People doing it",
        "value": "42,000"
      },
      {
        "label": "Jobs open each year",
        "value": "4,500"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$66K"
        }
      ],
      "best": [
        {
          "state": "California",
          "pay": "$125K"
        },
        {
          "state": "Washington",
          "pay": "$120K"
        },
        {
          "state": "Texas",
          "pay": "$95K"
        }
      ]
    },
    "knowAbout": [
      "Game systems and balance",
      "Math and probability",
      "Player psychology",
      "Programming basics",
      "Storytelling"
    ],
    "goodAt": [
      "Prototyping fast",
      "Clear design docs",
      "Taking playtest feedback",
      "Spreadsheet balancing",
      "A portfolio of small games"
    ],
    "software": [
      "Unity",
      "Unreal Engine",
      "Microsoft Excel",
      "Jira",
      "Miro"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "QA Tester",
        "pay": "$45K",
        "description": "You play builds all day and report every bug and rough edge.",
        "whatYouDo": [
          "Test builds",
          "Report bugs",
          "Reproduce issues",
          "Give feedback"
        ],
        "toGetHere": [
          "High school diploma",
          "Love of games"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Game Designer",
        "pay": "$98K",
        "description": "You design systems and levels and tune them through playtests.",
        "whatYouDo": [
          "Design systems",
          "Balance economies",
          "Write docs",
          "Run playtests"
        ],
        "toGetHere": [
          "Portfolio",
          "1 to 3 years experience"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Lead Game Designer",
        "pay": "$130K",
        "description": "You own the game's vision and lead the design team.",
        "whatYouDo": [
          "Set vision",
          "Lead designers",
          "Pitch to publishers",
          "Own the schedule"
        ],
        "toGetHere": [
          "5+ years experience",
          "Shipped titles"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Game Design"
        },
        {
          "name": "Computer Science"
        },
        {
          "name": "Interactive Media"
        }
      ],
      "where": [
        {
          "count": "380",
          "credential": "Bachelor's degree"
        },
        {
          "count": "90",
          "credential": "Master's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "quant": {
    "slug": "quant",
    "title": "Quant",
    "world": "Business & Money",
    "photo": "/images/app/poster-quant.png",
    "summary": "Uses math and code to find patterns in markets and trade on them.",
    "scenario": "Imagine a model that is right 51 percent of the time. At a million trades a day, that 1 percent is the whole business.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Master's degree"
      },
      {
        "label": "Typical pay",
        "value": "$118,000/year"
      },
      {
        "label": "People doing it",
        "value": "60,000"
      },
      {
        "label": "Jobs open each year",
        "value": "5,600"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$88K"
        }
      ],
      "best": [
        {
          "state": "New York",
          "pay": "$170K"
        },
        {
          "state": "Illinois",
          "pay": "$150K"
        },
        {
          "state": "Connecticut",
          "pay": "$145K"
        }
      ]
    },
    "knowAbout": [
      "Advanced math and statistics",
      "Programming",
      "Markets and trading",
      "Probability",
      "Machine learning"
    ],
    "goodAt": [
      "Rigorous thinking",
      "Python and C++",
      "Testing ideas against data",
      "Working fast and precisely",
      "A math-heavy degree"
    ],
    "software": [
      "Python",
      "C++",
      "kdb+/q",
      "Bloomberg Terminal",
      "Jupyter"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Quant Analyst",
        "pay": "$118K",
        "description": "You research signals and test strategies against historical data.",
        "whatYouDo": [
          "Research signals",
          "Backtest",
          "Clean data",
          "Write code"
        ],
        "toGetHere": [
          "Master's or PhD in a quantitative field"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Quant Researcher",
        "pay": "$180K",
        "description": "You own strategies from idea to live trading.",
        "whatYouDo": [
          "Design strategies",
          "Run live models",
          "Manage risk",
          "Publish research internally"
        ],
        "toGetHere": [
          "2 to 4 years experience",
          "Live track record"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Portfolio Manager",
        "pay": "$300K",
        "description": "You run a book of strategies and its profit and loss.",
        "whatYouDo": [
          "Allocate capital",
          "Manage risk",
          "Lead researchers",
          "Own P&L"
        ],
        "toGetHere": [
          "Years of results"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Mathematics"
        },
        {
          "name": "Statistics"
        },
        {
          "name": "Computer Science"
        },
        {
          "name": "Physics"
        }
      ],
      "where": [
        {
          "count": "620",
          "credential": "Master's degree"
        },
        {
          "count": "190",
          "credential": "Doctorate"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "management-analyst": {
    "slug": "management-analyst",
    "title": "Management Analyst",
    "world": "Business & Money",
    "photo": "/images/app/poster-management-analyst.png",
    "summary": "Studies how a company works and recommends how to do it better.",
    "scenario": "Imagine a hospital losing $4 million a year on scheduling. You have eight weeks to find out why and fix it.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$99,410/year"
      },
      {
        "label": "People doing it",
        "value": "1,032,000"
      },
      {
        "label": "Jobs open each year",
        "value": "95,700"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$74K"
        }
      ],
      "best": [
        {
          "state": "New York",
          "pay": "$120K"
        },
        {
          "state": "District of Columbia",
          "pay": "$118K"
        },
        {
          "state": "New Jersey",
          "pay": "$112K"
        }
      ]
    },
    "knowAbout": [
      "Business operations",
      "Data analysis",
      "Finance basics",
      "How people work in organizations",
      "Presenting"
    ],
    "goodAt": [
      "Asking sharp questions",
      "Structured thinking",
      "Spreadsheets and slides",
      "Interviewing people",
      "Travel and long weeks"
    ],
    "software": [
      "Microsoft Excel",
      "Microsoft PowerPoint",
      "Tableau",
      "Microsoft Teams",
      "Smartsheet"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Analyst",
        "pay": "$75K",
        "description": "You gather data and build the analysis behind a project's recommendations.",
        "whatYouDo": [
          "Gather data",
          "Build models",
          "Interview staff",
          "Draft slides"
        ],
        "toGetHere": [
          "Bachelor's degree",
          "Internship"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Consultant",
        "pay": "$110K",
        "description": "You own a workstream and present findings to clients.",
        "whatYouDo": [
          "Own workstreams",
          "Present to clients",
          "Manage analysts",
          "Design solutions"
        ],
        "toGetHere": [
          "2 to 3 years as analyst or an MBA"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Engagement Manager",
        "pay": "$160K",
        "description": "You run whole projects and the client relationship.",
        "whatYouDo": [
          "Run projects",
          "Manage teams",
          "Sell follow-on work",
          "Coach consultants"
        ],
        "toGetHere": [
          "5+ years experience"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Business Administration"
        },
        {
          "name": "Economics"
        },
        {
          "name": "Industrial Engineering"
        }
      ],
      "where": [
        {
          "count": "1,410",
          "credential": "Bachelor's degree"
        },
        {
          "count": "760",
          "credential": "Master's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "administrative-assistant": {
    "slug": "administrative-assistant",
    "title": "Administrative Assistant",
    "world": "Business & Money",
    "photo": "/images/app/poster-administrative-assistant.png",
    "summary": "Keeps an office and its people organized and on time.",
    "scenario": "Imagine three executives, one shared calendar, and a board meeting that moved to tomorrow. You are the one who makes it work.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "High school diploma or equivalent"
      },
      {
        "label": "Typical pay",
        "value": "$46,010/year"
      },
      {
        "label": "People doing it",
        "value": "3,300,000"
      },
      {
        "label": "Jobs open each year",
        "value": "316,000"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$40K"
        }
      ],
      "best": [
        {
          "state": "District of Columbia",
          "pay": "$60K"
        },
        {
          "state": "Massachusetts",
          "pay": "$55K"
        },
        {
          "state": "Washington",
          "pay": "$54K"
        }
      ]
    },
    "knowAbout": [
      "Office software",
      "Business writing",
      "Scheduling",
      "How a company runs",
      "Customer service"
    ],
    "goodAt": [
      "Organization",
      "Staying calm when plans change",
      "Clear writing",
      "Discretion",
      "Juggling many small tasks"
    ],
    "software": [
      "Microsoft Outlook",
      "Microsoft Excel",
      "Microsoft Word",
      "Zoom",
      "Google Workspace"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Receptionist",
        "pay": "$36K",
        "description": "You are the front door: phones, visitors and the day's flow.",
        "whatYouDo": [
          "Answer phones",
          "Greet visitors",
          "Schedule",
          "Handle mail"
        ],
        "toGetHere": [
          "High school diploma"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Administrative Assistant",
        "pay": "$46K",
        "description": "You support a team or department and keep it running.",
        "whatYouDo": [
          "Manage calendars",
          "Prepare documents",
          "Book travel",
          "Track expenses"
        ],
        "toGetHere": [
          "1 to 2 years experience",
          "Office software skills"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Executive Assistant",
        "pay": "$70K",
        "description": "You support senior leaders and run their world.",
        "whatYouDo": [
          "Manage executives' time",
          "Prepare briefings",
          "Coordinate meetings",
          "Handle confidential work"
        ],
        "toGetHere": [
          "5+ years experience"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Office Administration"
        },
        {
          "name": "Business Administration"
        }
      ],
      "where": [
        {
          "count": "540",
          "credential": "Certificate, under a year"
        },
        {
          "count": "710",
          "credential": "Associate's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "emergency-medicine-doctor": {
    "slug": "emergency-medicine-doctor",
    "title": "Emergency Medicine Doctor",
    "world": "Health & Medicine",
    "photo": "/images/app/poster-emergency-medicine-doctor.png",
    "summary": "Treats whoever comes through the ER doors, whatever is wrong.",
    "scenario": "Imagine a chest pain, a broken arm and a car crash arriving in the same ten minutes. You decide who is first.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Doctoral or professional degree"
      },
      {
        "label": "Typical pay",
        "value": "$239,200/year"
      },
      {
        "label": "People doing it",
        "value": "40,000"
      },
      {
        "label": "Jobs open each year",
        "value": "1,900"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$215K"
        }
      ],
      "best": [
        {
          "state": "Wisconsin",
          "pay": "$320K"
        },
        {
          "state": "Minnesota",
          "pay": "$305K"
        },
        {
          "state": "Indiana",
          "pay": "$300K"
        }
      ]
    },
    "knowAbout": [
      "Anatomy and biology",
      "Medications",
      "Trauma care",
      "Quick diagnosis",
      "Talking with scared people"
    ],
    "goodAt": [
      "Staying calm in chaos",
      "Fast decisions",
      "Working nights and weekends",
      "Teamwork",
      "Years of training"
    ],
    "software": [
      "Epic (EHR)",
      "PACS imaging software",
      "Dragon Medical",
      "Microsoft Excel",
      "UpToDate"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Medical Student",
        "pay": "",
        "description": "Four years of medical school before you see your own patients.",
        "whatYouDo": [
          "Study",
          "Rotate through specialties",
          "Pass boards",
          "Shadow"
        ],
        "toGetHere": [
          "Bachelor's degree",
          "MCAT"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Emergency Medicine Resident",
        "pay": "$65K",
        "description": "Three to four years of training in the ER under attendings.",
        "whatYouDo": [
          "See patients",
          "Run resuscitations",
          "Take call",
          "Present cases"
        ],
        "toGetHere": [
          "Medical degree",
          "Match into residency"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Attending Physician",
        "pay": "$239K",
        "description": "You run the ER shift and make the final calls.",
        "whatYouDo": [
          "Lead the department",
          "Make final calls",
          "Teach residents",
          "Manage the board"
        ],
        "toGetHere": [
          "Residency complete",
          "Board certification"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Biology"
        },
        {
          "name": "Chemistry"
        },
        {
          "name": "Pre-Medicine"
        }
      ],
      "where": [
        {
          "count": "1,300",
          "credential": "Bachelor's degree"
        },
        {
          "count": "190",
          "credential": "Doctorate"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "nurse-anesthetist": {
    "slug": "nurse-anesthetist",
    "title": "Nurse Anesthetist",
    "world": "Health & Medicine",
    "photo": "/images/app/poster-nurse-anesthetist.png",
    "summary": "Puts patients to sleep for surgery and keeps them safe until they wake up.",
    "scenario": "Imagine a patient's blood pressure dropping mid-surgery while the surgeon keeps working. Every second is yours to manage.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Master's degree"
      },
      {
        "label": "Typical pay",
        "value": "$212,650/year"
      },
      {
        "label": "People doing it",
        "value": "49,000"
      },
      {
        "label": "Jobs open each year",
        "value": "3,200"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$195K"
        }
      ],
      "best": [
        {
          "state": "Illinois",
          "pay": "$280K"
        },
        {
          "state": "Massachusetts",
          "pay": "$270K"
        },
        {
          "state": "Vermont",
          "pay": "$260K"
        }
      ]
    },
    "knowAbout": [
      "Anatomy and pharmacology",
      "Anesthesia drugs",
      "Monitoring equipment",
      "Surgery workflow",
      "Emergency response"
    ],
    "goodAt": [
      "Total focus for hours",
      "Steady hands",
      "Quick, calm decisions",
      "ICU nursing experience",
      "Passing the CRNA boards"
    ],
    "software": [
      "Epic (EHR)",
      "Anesthesia information systems",
      "Pyxis",
      "Microsoft Excel",
      "Cerner"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "ICU Nurse",
        "pay": "$86K",
        "description": "You care for the sickest patients and build the judgment anesthesia needs.",
        "whatYouDo": [
          "Manage critical patients",
          "Run drips",
          "Read monitors",
          "Respond to codes"
        ],
        "toGetHere": [
          "BSN",
          "RN license",
          "1 to 3 years in ICU"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Nurse Anesthesia Student",
        "pay": "",
        "description": "A three-year doctoral program with thousands of clinical hours.",
        "whatYouDo": [
          "Study pharmacology",
          "Give anesthesia under supervision",
          "Log cases",
          "Pass boards"
        ],
        "toGetHere": [
          "ICU experience",
          "Accepted to a CRNA program"
        ]
      },
      {
        "number": "3",
        "jobTitle": "CRNA",
        "pay": "$213K",
        "description": "You provide anesthesia on your own for surgeries of every kind.",
        "whatYouDo": [
          "Plan anesthesia",
          "Manage the airway",
          "Monitor the patient",
          "Handle emergencies"
        ],
        "toGetHere": [
          "Doctorate",
          "National certification"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Nursing (BSN)"
        },
        {
          "name": "Nurse Anesthesia"
        }
      ],
      "where": [
        {
          "count": "890",
          "credential": "Bachelor's degree"
        },
        {
          "count": "130",
          "credential": "Doctorate"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "lawyer": {
    "slug": "lawyer",
    "title": "Lawyer",
    "world": "Law, Safety & Justice",
    "photo": "/images/app/poster-lawyer.png",
    "summary": "Advises people and companies on the law and argues for them.",
    "scenario": "Imagine a client who will lose their house if you lose Tuesday's hearing. You have 400 pages to know cold.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Doctoral or professional degree"
      },
      {
        "label": "Typical pay",
        "value": "$145,760/year"
      },
      {
        "label": "People doing it",
        "value": "859,000"
      },
      {
        "label": "Jobs open each year",
        "value": "35,600"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$88K"
        }
      ],
      "best": [
        {
          "state": "District of Columbia",
          "pay": "$200K"
        },
        {
          "state": "California",
          "pay": "$190K"
        },
        {
          "state": "New York",
          "pay": "$185K"
        }
      ]
    },
    "knowAbout": [
      "Law and government",
      "Reading and writing well",
      "Logic and argument",
      "History",
      "Business"
    ],
    "goodAt": [
      "Reading fast and carefully",
      "Writing clearly",
      "Arguing a point",
      "Long hours",
      "Passing the bar exam"
    ],
    "software": [
      "Westlaw",
      "LexisNexis",
      "Microsoft Word",
      "Clio",
      "Adobe Acrobat"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Law Clerk",
        "pay": "$50K",
        "description": "You research and draft memos for practicing attorneys while in school.",
        "whatYouDo": [
          "Research cases",
          "Draft memos",
          "Review documents",
          "Observe court"
        ],
        "toGetHere": [
          "Enrolled in law school"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Associate Attorney",
        "pay": "$120K",
        "description": "You handle your own cases under a partner's supervision.",
        "whatYouDo": [
          "Manage cases",
          "Draft filings",
          "Meet clients",
          "Appear in court"
        ],
        "toGetHere": [
          "Law degree",
          "Bar admission"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Partner",
        "pay": "$250K",
        "description": "You bring in clients, lead cases and share in the firm.",
        "whatYouDo": [
          "Win clients",
          "Lead major cases",
          "Mentor associates",
          "Run the practice"
        ],
        "toGetHere": [
          "7+ years experience",
          "Book of business"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Political Science"
        },
        {
          "name": "Pre-Law"
        },
        {
          "name": "Law (JD)"
        }
      ],
      "where": [
        {
          "count": "1,500",
          "credential": "Bachelor's degree"
        },
        {
          "count": "200",
          "credential": "Doctorate"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "airline-pilot": {
    "slug": "airline-pilot",
    "title": "Airline Pilot",
    "world": "Driving, Flying & Shipping",
    "photo": "/images/app/poster-airline-pilot.png",
    "summary": "Flies passengers and cargo safely from one airport to another.",
    "scenario": "Imagine 180 people behind you, a line of thunderstorms ahead, and a decision to make in the next two minutes.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$219,140/year"
      },
      {
        "label": "People doing it",
        "value": "96,000"
      },
      {
        "label": "Jobs open each year",
        "value": "18,500"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$150K"
        }
      ],
      "best": [
        {
          "state": "Washington",
          "pay": "$260K"
        },
        {
          "state": "New York",
          "pay": "$240K"
        },
        {
          "state": "Illinois",
          "pay": "$235K"
        }
      ]
    },
    "knowAbout": [
      "Aerodynamics and weather",
      "Navigation",
      "Aircraft systems",
      "Regulations",
      "Teamwork in the cockpit"
    ],
    "goodAt": [
      "Calm, methodical decisions",
      "Hand-eye coordination",
      "Checklists every time",
      "Health and eyesight standards",
      "1,500 flight hours"
    ],
    "software": [
      "ForeFlight",
      "Jeppesen FliteDeck",
      "Electronic Flight Bag",
      "Garmin Pilot",
      "Crew scheduling software"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Flight Instructor",
        "pay": "$45K",
        "description": "You build hours teaching other students to fly.",
        "whatYouDo": [
          "Teach students",
          "Build hours",
          "Fly daily",
          "Pass checkrides"
        ],
        "toGetHere": [
          "Commercial license",
          "Instructor certificate"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Regional First Officer",
        "pay": "$90K",
        "description": "Your first airline job, flying smaller jets as the co-pilot.",
        "whatYouDo": [
          "Fly regional routes",
          "Run checklists",
          "Coordinate with the captain",
          "Handle weather"
        ],
        "toGetHere": [
          "1,500 hours",
          "ATP certificate"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Major Airline Captain",
        "pay": "$250K",
        "description": "You command large jets on the longest routes.",
        "whatYouDo": [
          "Command the aircraft",
          "Lead the crew",
          "Make final calls",
          "Mentor first officers"
        ],
        "toGetHere": [
          "Years of seniority",
          "Type ratings"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Aviation"
        },
        {
          "name": "Aeronautical Science"
        }
      ],
      "where": [
        {
          "count": "120",
          "credential": "Associate's degree"
        },
        {
          "count": "140",
          "credential": "Bachelor's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "therapist": {
    "slug": "therapist",
    "title": "Therapist",
    "world": "Counseling & Social Work",
    "photo": "/images/app/poster-therapist.png",
    "summary": "Helps people work through anxiety, grief, relationships and hard seasons.",
    "scenario": "Imagine a client who has not slept in a week finally saying the thing they came to say. Your next sentence matters.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Master's degree"
      },
      {
        "label": "Typical pay",
        "value": "$58,510/year"
      },
      {
        "label": "People doing it",
        "value": "449,800"
      },
      {
        "label": "Jobs open each year",
        "value": "48,900"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$50K"
        }
      ],
      "best": [
        {
          "state": "Utah",
          "pay": "$85K"
        },
        {
          "state": "New Jersey",
          "pay": "$80K"
        },
        {
          "state": "Rhode Island",
          "pay": "$78K"
        }
      ]
    },
    "knowAbout": [
      "Psychology",
      "How people change",
      "Ethics and confidentiality",
      "Mental health conditions",
      "Listening"
    ],
    "goodAt": [
      "Empathy without burning out",
      "Patience",
      "Keeping boundaries",
      "Clear notes",
      "A state license after supervised hours"
    ],
    "software": [
      "SimplePractice",
      "TherapyNotes",
      "Zoom",
      "Microsoft Word",
      "Google Workspace"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Counseling Intern",
        "pay": "",
        "description": "You see clients under supervision while finishing your master's.",
        "whatYouDo": [
          "See clients",
          "Get supervision",
          "Write notes",
          "Learn methods"
        ],
        "toGetHere": [
          "Enrolled in a counseling master's"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Associate Therapist",
        "pay": "$52K",
        "description": "You counsel clients while logging the supervised hours for your license.",
        "whatYouDo": [
          "Carry a caseload",
          "Log hours",
          "Consult with supervisor",
          "Run groups"
        ],
        "toGetHere": [
          "Master's degree",
          "Provisional license"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Licensed Therapist",
        "pay": "$75K",
        "description": "You practice on your own, in an agency or your own office.",
        "whatYouDo": [
          "Run a caseload",
          "Bill insurance",
          "Specialize",
          "Supervise others"
        ],
        "toGetHere": [
          "Licensure",
          "2,000 to 4,000 supervised hours"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Counseling"
        },
        {
          "name": "Psychology"
        },
        {
          "name": "Social Work"
        }
      ],
      "where": [
        {
          "count": "810",
          "credential": "Master's degree"
        },
        {
          "count": "120",
          "credential": "Doctorate"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "database-architect": {
    "slug": "database-architect",
    "title": "Database Architect",
    "world": "Tech & Engineering",
    "photo": "/images/app/poster-database-architect.png",
    "summary": "Designs how a company stores and finds its most important data.",
    "scenario": "Imagine Black Friday traffic hitting a database you designed. If it holds, nobody notices. If it does not, everyone does.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$134,700/year"
      },
      {
        "label": "People doing it",
        "value": "82,000"
      },
      {
        "label": "Jobs open each year",
        "value": "7,900"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$96K"
        }
      ],
      "best": [
        {
          "state": "Washington",
          "pay": "$165K"
        },
        {
          "state": "California",
          "pay": "$160K"
        },
        {
          "state": "New Jersey",
          "pay": "$150K"
        }
      ]
    },
    "knowAbout": [
      "Databases and SQL",
      "How data is structured",
      "Programming",
      "Security",
      "Cloud systems"
    ],
    "goodAt": [
      "Thinking in systems",
      "Precision",
      "Planning for growth",
      "Explaining tradeoffs",
      "Years of hands-on work"
    ],
    "software": [
      "PostgreSQL",
      "Microsoft SQL Server",
      "Amazon Web Services",
      "Snowflake",
      "Git / GitHub"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Database Administrator",
        "pay": "$80K",
        "description": "You keep databases running, backed up and fast.",
        "whatYouDo": [
          "Tune performance",
          "Run backups",
          "Manage access",
          "Fix outages"
        ],
        "toGetHere": [
          "Bachelor's degree",
          "SQL skills"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Data Engineer",
        "pay": "$110K",
        "description": "You build the pipelines that move data where it needs to go.",
        "whatYouDo": [
          "Build pipelines",
          "Model data",
          "Automate",
          "Monitor quality"
        ],
        "toGetHere": [
          "2 to 4 years experience"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Database Architect",
        "pay": "$135K",
        "description": "You design the data platform and set the standards everyone builds on.",
        "whatYouDo": [
          "Design schemas",
          "Choose platforms",
          "Set standards",
          "Guide teams"
        ],
        "toGetHere": [
          "7+ years experience"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Computer Science"
        },
        {
          "name": "Information Systems"
        },
        {
          "name": "Data Engineering"
        }
      ],
      "where": [
        {
          "count": "1,210",
          "credential": "Bachelor's degree"
        },
        {
          "count": "560",
          "credential": "Master's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "sound-engineering-technician": {
    "slug": "sound-engineering-technician",
    "title": "Sound Engineering Technician",
    "world": "Arts, Media & Sport",
    "photo": "/images/app/poster-sound-engineering-technician.png",
    "summary": "Records, mixes and runs the sound for music, film and live shows.",
    "scenario": "Imagine 20,000 people in an arena and the lead vocal mic cutting out on the first chorus. You have one bar to fix it.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Postsecondary certificate"
      },
      {
        "label": "Typical pay",
        "value": "$59,430/year"
      },
      {
        "label": "People doing it",
        "value": "20,300"
      },
      {
        "label": "Jobs open each year",
        "value": "2,200"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$44K"
        }
      ],
      "best": [
        {
          "state": "California",
          "pay": "$85K"
        },
        {
          "state": "New York",
          "pay": "$80K"
        },
        {
          "state": "District of Columbia",
          "pay": "$75K"
        }
      ]
    },
    "knowAbout": [
      "Sound and acoustics",
      "Music",
      "Electronics",
      "Recording software",
      "How live shows run"
    ],
    "goodAt": [
      "Sharp ears",
      "Calm under pressure",
      "Long, odd hours",
      "Cable and gear know-how",
      "Working with artists"
    ],
    "software": [
      "Pro Tools",
      "Ableton Live",
      "Logic Pro",
      "Dante Controller",
      "Microsoft Excel"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Audio Assistant",
        "pay": "$35K",
        "description": "You set up gear, run cables and learn by watching the engineer.",
        "whatYouDo": [
          "Set up gear",
          "Run cables",
          "Label and log",
          "Assist sessions"
        ],
        "toGetHere": [
          "Certificate or on-the-job start"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Sound Engineer",
        "pay": "$59K",
        "description": "You run sessions and shows on your own.",
        "whatYouDo": [
          "Mix live or in studio",
          "Set up systems",
          "Solve problems fast",
          "Work with artists"
        ],
        "toGetHere": [
          "2 to 4 years assisting"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Head Engineer",
        "pay": "$85K",
        "description": "You lead the audio team for a venue, studio or tour.",
        "whatYouDo": [
          "Design systems",
          "Lead crews",
          "Advance shows",
          "Own the sound"
        ],
        "toGetHere": [
          "Years of shows",
          "Reputation"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Audio Engineering"
        },
        {
          "name": "Music Technology"
        }
      ],
      "where": [
        {
          "count": "210",
          "credential": "Certificate, under a year"
        },
        {
          "count": "160",
          "credential": "Associate's degree"
        },
        {
          "count": "120",
          "credential": "Bachelor's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "drone-pilot": {
    "slug": "drone-pilot",
    "title": "Drone Pilot",
    "world": "Driving, Flying & Shipping",
    "photo": "/images/app/poster-drone-pilot.png",
    "summary": "Flies drones for filming, inspections, mapping and deliveries.",
    "scenario": "Imagine a bridge inspection 200 feet up in a crosswind, with an engineer watching your feed for a crack the size of a pencil.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "High school diploma or equivalent"
      },
      {
        "label": "Typical pay",
        "value": "$113,080/year"
      },
      {
        "label": "People doing it",
        "value": "50,000"
      },
      {
        "label": "Jobs open each year",
        "value": "5,000"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$70K"
        }
      ],
      "best": [
        {
          "state": "Texas",
          "pay": "$130K"
        },
        {
          "state": "California",
          "pay": "$125K"
        },
        {
          "state": "Florida",
          "pay": "$115K"
        }
      ]
    },
    "knowAbout": [
      "Aviation rules",
      "Weather",
      "Cameras and sensors",
      "Maps and GPS",
      "Batteries and electronics"
    ],
    "goodAt": [
      "Steady hands",
      "Planning a flight",
      "Reading weather",
      "Safety first",
      "An FAA Part 107 certificate"
    ],
    "software": [
      "DJI Pilot",
      "Pix4D",
      "DroneDeploy",
      "Adobe Premiere Pro",
      "Litchi"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Drone Operator",
        "pay": "$45K",
        "description": "You fly missions for a company under set plans.",
        "whatYouDo": [
          "Fly missions",
          "Capture footage or data",
          "Maintain drones",
          "Log flights"
        ],
        "toGetHere": [
          "Part 107 certificate"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Commercial Drone Pilot",
        "pay": "$80K",
        "description": "You plan and fly complex jobs for clients.",
        "whatYouDo": [
          "Plan flights",
          "Get waivers",
          "Fly complex jobs",
          "Deliver data"
        ],
        "toGetHere": [
          "1 to 3 years flying",
          "Specialty training"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Flight Operations Lead",
        "pay": "$113K",
        "description": "You run a drone program and its pilots.",
        "whatYouDo": [
          "Lead pilots",
          "Set procedures",
          "Manage compliance",
          "Win contracts"
        ],
        "toGetHere": [
          "Years of experience",
          "Aviation background helps"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Unmanned Aircraft Systems"
        },
        {
          "name": "Aviation"
        }
      ],
      "where": [
        {
          "count": "70",
          "credential": "Certificate, under a year"
        },
        {
          "count": "50",
          "credential": "Associate's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "jewelry-designer": {
    "slug": "jewelry-designer",
    "title": "Jewelry Designer",
    "world": "Factories & Making Things",
    "photo": "/images/app/poster-jewelry-designer.png",
    "summary": "Designs and makes rings, necklaces and pieces people keep for life.",
    "scenario": "Imagine an engagement ring due Saturday. The stone is set, the band is polished, and you see a scratch no one else would notice.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "High school diploma or equivalent"
      },
      {
        "label": "Typical pay",
        "value": "$47,450/year"
      },
      {
        "label": "People doing it",
        "value": "32,500"
      },
      {
        "label": "Jobs open each year",
        "value": "3,300"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$38K"
        }
      ],
      "best": [
        {
          "state": "New York",
          "pay": "$65K"
        },
        {
          "state": "Massachusetts",
          "pay": "$58K"
        },
        {
          "state": "California",
          "pay": "$56K"
        }
      ]
    },
    "knowAbout": [
      "Design and drawing",
      "Metals and gems",
      "Tools and techniques",
      "Fashion trends",
      "Running a small business"
    ],
    "goodAt": [
      "Steady hands",
      "Patience for tiny work",
      "An eye for detail",
      "Sketching ideas",
      "Talking with customers"
    ],
    "software": [
      "Rhino 3D",
      "Adobe Illustrator",
      "MatrixGold",
      "Shopify",
      "Instagram"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Bench Apprentice",
        "pay": "$30K",
        "description": "You learn soldering, setting and polishing at the bench.",
        "whatYouDo": [
          "Polish",
          "Solder",
          "Size rings",
          "Learn setting"
        ],
        "toGetHere": [
          "High school diploma",
          "Portfolio or classes"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Bench Jeweler",
        "pay": "$47K",
        "description": "You make and repair pieces on your own.",
        "whatYouDo": [
          "Fabricate pieces",
          "Set stones",
          "Repair",
          "Work with customers"
        ],
        "toGetHere": [
          "2 to 4 years at the bench"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Jewelry Designer",
        "pay": "$70K",
        "description": "You design collections and often run your own brand.",
        "whatYouDo": [
          "Design collections",
          "Source stones",
          "Sell work",
          "Run the business"
        ],
        "toGetHere": [
          "Years of bench work",
          "Design training"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Jewelry Design"
        },
        {
          "name": "Metalsmithing"
        },
        {
          "name": "Fine Arts"
        }
      ],
      "where": [
        {
          "count": "60",
          "credential": "Certificate, under a year"
        },
        {
          "count": "90",
          "credential": "Bachelor's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "pediatric-surgeon": {
    "slug": "pediatric-surgeon",
    "title": "Pediatric Surgeon",
    "world": "Health & Medicine",
    "photo": "/images/app/poster-pediatric-surgeon.png",
    "summary": "Operates on children, from newborns to teenagers.",
    "scenario": "Imagine a newborn weighing four pounds and a surgery where the margin for error is measured in millimeters.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Doctoral or professional degree"
      },
      {
        "label": "Typical pay",
        "value": "$559,000/year"
      },
      {
        "label": "People doing it",
        "value": "5,000"
      },
      {
        "label": "Jobs open each year",
        "value": "200"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$480K"
        }
      ],
      "best": [
        {
          "state": "Georgia",
          "pay": "$600K"
        },
        {
          "state": "Wisconsin",
          "pay": "$590K"
        },
        {
          "state": "Texas",
          "pay": "$580K"
        }
      ]
    },
    "knowAbout": [
      "Anatomy and biology",
      "Child development",
      "Surgical technique",
      "Imaging",
      "Talking with parents"
    ],
    "goodAt": [
      "Steady hands",
      "Years of training",
      "Calm in the OR",
      "Stamina",
      "Compassion with families"
    ],
    "software": [
      "Epic (EHR)",
      "PACS imaging software",
      "Surgical navigation systems",
      "OR scheduling systems",
      "Microsoft Excel"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Medical Student",
        "pay": "",
        "description": "Four years of medical school before you touch a scalpel professionally.",
        "whatYouDo": [
          "Study",
          "Rotate through specialties",
          "Pass boards",
          "Shadow"
        ],
        "toGetHere": [
          "Bachelor's degree",
          "MCAT"
        ]
      },
      {
        "number": "2",
        "jobTitle": "General Surgery Resident",
        "pay": "$65K",
        "description": "Five years of surgical training in a hospital.",
        "whatYouDo": [
          "Operate under supervision",
          "Take call",
          "Round on patients",
          "Present cases"
        ],
        "toGetHere": [
          "Medical degree",
          "Match into surgery"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Pediatric Surgeon",
        "pay": "$559K",
        "description": "You complete a two-year fellowship and operate on children on your own.",
        "whatYouDo": [
          "Operate",
          "Consult with families",
          "Lead the OR team",
          "Teach residents"
        ],
        "toGetHere": [
          "Pediatric surgery fellowship",
          "Board certification"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Biology"
        },
        {
          "name": "Chemistry"
        },
        {
          "name": "Pre-Medicine"
        }
      ],
      "where": [
        {
          "count": "1,300",
          "credential": "Bachelor's degree"
        },
        {
          "count": "190",
          "credential": "Doctorate"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "purchasing-manager": {
    "slug": "purchasing-manager",
    "title": "Purchasing Manager",
    "world": "Business & Money",
    "photo": "/images/app/poster-purchasing-manager.png",
    "summary": "Decides what a company buys, from whom, and at what price.",
    "scenario": "Imagine a supplier calling to say the parts your factory needs tomorrow are stuck at a port. You have four hours.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$148,000/year"
      },
      {
        "label": "People doing it",
        "value": "79,600"
      },
      {
        "label": "Jobs open each year",
        "value": "6,300"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$105K"
        }
      ],
      "best": [
        {
          "state": "New Jersey",
          "pay": "$170K"
        },
        {
          "state": "New York",
          "pay": "$165K"
        },
        {
          "state": "Delaware",
          "pay": "$160K"
        }
      ]
    },
    "knowAbout": [
      "Business and economics",
      "Negotiation",
      "Supply chains",
      "Contracts",
      "Spreadsheets"
    ],
    "goodAt": [
      "Negotiating",
      "Reading contracts",
      "Building supplier relationships",
      "Analyzing costs",
      "Staying calm when supply breaks"
    ],
    "software": [
      "SAP Ariba",
      "Coupa",
      "Microsoft Excel",
      "Oracle",
      "Microsoft Teams"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Buyer",
        "pay": "$60K",
        "description": "You place orders and track deliveries for a category.",
        "whatYouDo": [
          "Place orders",
          "Track deliveries",
          "Compare quotes",
          "Update systems"
        ],
        "toGetHere": [
          "Bachelor's degree"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Senior Buyer",
        "pay": "$90K",
        "description": "You negotiate contracts and own supplier relationships.",
        "whatYouDo": [
          "Negotiate contracts",
          "Manage suppliers",
          "Cut costs",
          "Handle disruptions"
        ],
        "toGetHere": [
          "3 to 5 years experience",
          "CPSM helps"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Purchasing Manager",
        "pay": "$148K",
        "description": "You run purchasing for a company and its strategy.",
        "whatYouDo": [
          "Set strategy",
          "Lead buyers",
          "Approve major deals",
          "Manage risk"
        ],
        "toGetHere": [
          "7+ years experience"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Supply Chain Management"
        },
        {
          "name": "Business Administration"
        },
        {
          "name": "Economics"
        }
      ],
      "where": [
        {
          "count": "420",
          "credential": "Bachelor's degree"
        },
        {
          "count": "210",
          "credential": "Master's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "cardiologist": {
    "slug": "cardiologist",
    "title": "Cardiologist",
    "world": "Health & Medicine",
    "photo": "/images/app/poster-cardiologist.png",
    "summary": "Diagnoses and treats problems with the heart.",
    "scenario": "Imagine reading a stress test at 7 a.m. and seeing the pattern that means this patient needs a stent today, not next month.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Doctoral or professional degree"
      },
      {
        "label": "Typical pay",
        "value": "$496,000/year"
      },
      {
        "label": "People doing it",
        "value": "20,000"
      },
      {
        "label": "Jobs open each year",
        "value": "900"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$430K"
        }
      ],
      "best": [
        {
          "state": "Wisconsin",
          "pay": "$560K"
        },
        {
          "state": "Missouri",
          "pay": "$540K"
        },
        {
          "state": "Texas",
          "pay": "$530K"
        }
      ]
    },
    "knowAbout": [
      "Anatomy and physiology",
      "Medications",
      "Imaging and tests",
      "Statistics",
      "Talking with patients"
    ],
    "goodAt": [
      "Pattern recognition",
      "Steady hands for procedures",
      "Years of training",
      "Calm under pressure",
      "Explaining risk clearly"
    ],
    "software": [
      "Epic (EHR)",
      "PACS imaging software",
      "Echo and ECG systems",
      "Dragon Medical",
      "UpToDate"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Medical Student",
        "pay": "",
        "description": "Four years of medical school before you see your own patients.",
        "whatYouDo": [
          "Study",
          "Rotate through specialties",
          "Pass boards",
          "Shadow"
        ],
        "toGetHere": [
          "Bachelor's degree",
          "MCAT"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Internal Medicine Resident",
        "pay": "$65K",
        "description": "Three years of hospital training before you specialize.",
        "whatYouDo": [
          "See patients",
          "Take call",
          "Learn procedures",
          "Present cases"
        ],
        "toGetHere": [
          "Medical degree",
          "Match into residency"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Cardiologist",
        "pay": "$496K",
        "description": "You finish a three-year fellowship and treat hearts on your own.",
        "whatYouDo": [
          "Diagnose",
          "Perform procedures",
          "Manage chronic disease",
          "Teach"
        ],
        "toGetHere": [
          "Cardiology fellowship",
          "Board certification"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Biology"
        },
        {
          "name": "Chemistry"
        },
        {
          "name": "Pre-Medicine"
        }
      ],
      "where": [
        {
          "count": "1,300",
          "credential": "Bachelor's degree"
        },
        {
          "count": "190",
          "credential": "Doctorate"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "public-relations-manager": {
    "slug": "public-relations-manager",
    "title": "Public Relations Manager",
    "world": "Business & Money",
    "photo": "/images/app/poster-public-relations-manager.png",
    "summary": "Shapes how the public sees a company or person.",
    "scenario": "Imagine a product recall trending on social media by 9 a.m. Your statement goes out at 10 and every reporter is quoting it.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Bachelor's degree"
      },
      {
        "label": "Typical pay",
        "value": "$146,000/year"
      },
      {
        "label": "People doing it",
        "value": "90,000"
      },
      {
        "label": "Jobs open each year",
        "value": "7,000"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$98K"
        }
      ],
      "best": [
        {
          "state": "District of Columbia",
          "pay": "$190K"
        },
        {
          "state": "New York",
          "pay": "$185K"
        },
        {
          "state": "Virginia",
          "pay": "$170K"
        }
      ]
    },
    "knowAbout": [
      "Writing and speaking",
      "Media and news",
      "Social platforms",
      "Business strategy",
      "Psychology of audiences"
    ],
    "goodAt": [
      "Writing fast and well",
      "Staying calm in a crisis",
      "Building relationships with reporters",
      "Judgment",
      "Presenting to executives"
    ],
    "software": [
      "Cision",
      "Muck Rack",
      "Sprout Social",
      "Microsoft PowerPoint",
      "Slack"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "PR Coordinator",
        "pay": "$50K",
        "description": "You draft releases, track coverage and support the team.",
        "whatYouDo": [
          "Draft releases",
          "Track coverage",
          "Build media lists",
          "Support events"
        ],
        "toGetHere": [
          "Bachelor's degree",
          "Internship"
        ]
      },
      {
        "number": "2",
        "jobTitle": "PR Manager",
        "pay": "$85K",
        "description": "You own campaigns and relationships with reporters.",
        "whatYouDo": [
          "Run campaigns",
          "Pitch stories",
          "Handle inquiries",
          "Advise teams"
        ],
        "toGetHere": [
          "3 to 5 years experience"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Director of Communications",
        "pay": "$146K",
        "description": "You set the communications strategy and speak for the company.",
        "whatYouDo": [
          "Set strategy",
          "Lead crisis response",
          "Advise executives",
          "Lead the team"
        ],
        "toGetHere": [
          "8+ years experience"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Public Relations"
        },
        {
          "name": "Communications"
        },
        {
          "name": "Journalism"
        }
      ],
      "where": [
        {
          "count": "720",
          "credential": "Bachelor's degree"
        },
        {
          "count": "240",
          "credential": "Master's degree"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  },
  "veterinarian": {
    "slug": "veterinarian",
    "title": "Veterinarian",
    "world": "Farming, Animals & Nature",
    "photo": "/images/app/poster-veterinarian.png",
    "summary": "Cares for animals' health, from checkups to surgery.",
    "scenario": "Imagine a golden retriever that swallowed a sock and an owner in tears. You have an X-ray and a decision to make.",
    "facts": [
      {
        "label": "Typical degree",
        "value": "Doctoral or professional degree"
      },
      {
        "label": "Typical pay",
        "value": "$130,000/year"
      },
      {
        "label": "People doing it",
        "value": "89,200"
      },
      {
        "label": "Jobs open each year",
        "value": "4,300"
      }
    ],
    "payByState": {
      "title": "Pay by state",
      "yourStates": [
        {
          "state": "South Dakota",
          "pay": "$105K"
        }
      ],
      "best": [
        {
          "state": "California",
          "pay": "$160K"
        },
        {
          "state": "New Jersey",
          "pay": "$150K"
        },
        {
          "state": "New York",
          "pay": "$145K"
        }
      ]
    },
    "knowAbout": [
      "Animal biology",
      "Medications and surgery",
      "Diseases",
      "Talking with owners",
      "Running a clinic"
    ],
    "goodAt": [
      "Compassion",
      "Steady hands",
      "Reading animals that cannot talk",
      "Long hours on your feet",
      "Eight years of school"
    ],
    "software": [
      "ezyVet",
      "IDEXX Cornerstone",
      "Microsoft Excel",
      "AVImark",
      "Zoom"
    ],
    "ladder": [
      {
        "number": "1",
        "jobTitle": "Veterinary Technician",
        "pay": "$43K",
        "description": "You assist in exams, run tests and care for animals in the clinic.",
        "whatYouDo": [
          "Assist exams",
          "Run lab tests",
          "Give medications",
          "Comfort animals"
        ],
        "toGetHere": [
          "Associate's degree",
          "Vet tech license"
        ]
      },
      {
        "number": "2",
        "jobTitle": "Associate Veterinarian",
        "pay": "$110K",
        "description": "Your first job after vet school, seeing your own patients in a practice.",
        "whatYouDo": [
          "Examine",
          "Diagnose",
          "Perform surgery",
          "Advise owners"
        ],
        "toGetHere": [
          "DVM degree",
          "State license"
        ]
      },
      {
        "number": "3",
        "jobTitle": "Practice Owner",
        "pay": "$160K",
        "description": "You run your own clinic and its team.",
        "whatYouDo": [
          "Run the clinic",
          "Lead staff",
          "Manage finances",
          "Set standards of care"
        ],
        "toGetHere": [
          "Years of practice",
          "Business skills"
        ]
      }
    ],
    "education": {
      "studies": [
        {
          "name": "Animal Science"
        },
        {
          "name": "Biology"
        },
        {
          "name": "Veterinary Medicine (DVM)"
        }
      ],
      "where": [
        {
          "count": "560",
          "credential": "Bachelor's degree"
        },
        {
          "count": "33",
          "credential": "Doctorate"
        }
      ]
    },
    "sources": "Job description and skills from O*NET (USDOL/ETA). Pay and growth from the U.S. Bureau of Labor Statistics."
  }
};
