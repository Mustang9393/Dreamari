...
export const CHANGE_NOTES: Record<CounselorView, ChangeNote> = {
  overview: {
    changed: ["Attention list leads the page, students ranked by severity with the reason", "One hero card, plain glass elsewhere; one blue hue plus status colors", "Pathways are Build's own interest worlds, in the same colors those worlds wear everywhere else in the student app, six largest plus Other", "Career Readiness and Academic Readiness are the reference's own two bar charts, same grouping and grades, in this dashboard's solid bar style; a real 0% now draws a visible bar instead of reading as an empty gap", "Every card opens its screen; every clickable row carries an arrow (leaves the screen) or a chevron (opens here)"],
    why: "The reference showed three equal donuts, its own pathway vocabulary that doesn't match the rest of Dreamari, and no way to act.",
    better: "Worst first, one glance, one click to the student, the same pathway names and colors a student sees in Build, and readiness charts that never hide a real zero.",
