...
// FROZEN SNAPSHOT, Express mode only. This is IB Level 1 exactly as it
// played before the 20 Sept 2026 three-act/binary-scoring rebuild
// (ib-level-1.ts), preserved verbatim from commit f999ea2b -- direct
// instruction, 21 Sept 2026: "the express mode changed to the full game
// with recent updates... I need it reverted back to what express mode
// was before yesterday's play update. ONLY EXPRESS MODE should revert."
//
// Full mode is untouched by this file -- it keeps playing the current
// ib-level-1.ts rebuild. IB_LEVEL_1.expressSource points here so the
// Express derivation (src/app/play/[game]/page.tsx) builds from THIS
// object's own beats + expressCut instead of the live Full-mode beats.
//
// Every beat id below is prefixed L1E- (was L1-) so it can never collide
// with a same-numbered but semantically DIFFERENT beat in the current
// ib-level-1.ts -- the two files now share an id NAMESPACE ("L1-" vs
// "L1E-"), not any actual content. locations.ts has a matching L1E-*
// block for exactly this reason; do not merge these ids back into L1-*.
//
// Original header, kept for provenance: from DreamAri_IB_Levels1-3_Handoff
// (2).xlsx (Aug 31), tab "Level 1 Intern" -- story cards split to one idea
// per screen, teach-then-check pacing, the two-card character system with
// the power-ladder graphic, drag interactions, Tap to Reveal teaching
// screens, the client renamed to Maison Laurent, Jordan reframed from
// rivalry to composure, and the rebalanced review-week beat. Copy is
// verbatim from that sheet. Scores are that sheet's production column, ten
// scored beats at 10% each. `feedback` strings are not displayed (the card
// shows the derived headline plus the chosen option's own why line), so
// they are left empty. Art is reused, never invented: the hand-drawn hero
// scenes (l1-12/l1-13.webp) are the same files the current rebuild also
// reuses; everything else resolves a Cobalt Capital location keyed by
// these L1E- ids.

const ART = "/images/play/ib";

export const IB_LEVEL_1_EXPRESS_LEGACY: Level = {
  id: "ib-l1",
  n: 1,
  role: "Intern",
  ...
  // guessing at a fix.
  expressCut: ["L1E-02", "L1E-03", "L1E-03b", "L1E-04", "L1E-06", "L1E-07", "L1E-09", "L1E-10", "L1E-17", "L1E-18", "L1E-19", "L1E-20", "L1E-21", "L1E-22", "L1E-23"],
  beats: [
