...
export function flyXp({ from, amount, milestone, tone }: { from: Element | Point | null | undefined; amount: number; milestone: string; /** CSS color for the number; defaults to the app accent */ tone?: string }): boolean {
  ...
  Object.assign(el.style, {
    ...
    // body { zoom: 1.1/1.25 } above 1441px/1800px). start/end above are
    // already TRUE post-zoom screen coordinates from getBoundingClientRect(),
    // so left unreset they'd be re-zoomed a second time on render. `zoom: 1`
    // alone is NOT enough -- zoom compounds down the tree (body 1.25 x this
    // element's 1 is still 1.25 effective), it doesn't reset to 1. The
    ...
    // purpose. Same fix as the shared Portal in CareerReport.tsx, which this
    // element can't use since it isn't a React portal.
